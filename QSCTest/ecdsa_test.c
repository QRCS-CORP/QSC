...
bool qsctest_ecdsa_kat_test()
{
	...
	hex_to_bin("B5076A8474A832DAEE4DD5B4040983B6623B5F344ACA57D4D6EE4BAF3F259E6E", kpk[4], sizeof(kpk[4]));
	hex_to_bin("421151A459FAEADE3D247115F94AEDAE42318124095AFABE4D1451A559FAEDEE", ksd, sizeof(ksd));
	hex_to_bin("421151A459FAEADE3D247115F94AEDAE42318124095AFABE4D1451A559FAEDEE"
		"B5076A8474A832DAEE4DD5B4040983B6623B5F344ACA57D4D6EE4BAF3F259E6E", ksk[4], sizeof(ksk[4]));

	hex_to_bin("A750C232933DC14B1184D86D8B4CE72E16D69744BA69818B6AC33B1D823BB2C3", kms0, sizeof(kms0));
	hex_to_bin("4BAFDAC9099D4057ED6DD08BCAEE8756E9A40F2CB9598020EB95019528409BBE"
		"A38B384A59F119F57297BFB2FA142FC7BB1D90DBDDDE772BCDE48C5670D5FA13", kms1, sizeof(kms1));
	hex_to_bin("FE6C1A31068E332D12AAB37D99406568DEAA36BDB277CEE55304633BD0A267A8"
	...
	hex_to_bin("F7E67D982A2FF93ECDA4087152B4864C943B1BA7021F5407043CCB4253D348C2"
	...
	hex_to_bin("B49F3A78B1C6A7FCA8F3466F33BC0E929F01FBA04306C2A7465F46C3759316D9", kpk[0], sizeof(kpk[0]));
	hex_to_bin("F73FA076F84B6DB675A5FDA5AD67E351A41E8E7F29ADD16809CA010387E9C6CC", kpk[1], sizeof(kpk[1]));
	hex_to_bin("6FF19B1F18D64851D5C74845C6407F0BF596A52E385E020127E83E54CFF5AC19", kpk[2], sizeof(kpk[2]));
	hex_to_bin("98BE21001993A7EB1A1277FF74C15504183D25FDFCC05F0D4DEA892F6E301890", kpk[3], sizeof(kpk[3]));

	hex_to_bin("8ED7A797B9CEA8A8370D419136BCDF683B759D2E3C6947F17E13E2485AA9D420"
		"B49F3A78B1C6A7FCA8F3466F33BC0E929F01FBA04306C2A7465F46C3759316D9", ksk[0], sizeof(ksk[0]));
	hex_to_bin("BA4D6E67B2CE67A1E44326494044F37A442F3B81725BC1F9341462718B55EE20"
		"F73FA076F84B6DB675A5FDA5AD67E351A41E8E7F29ADD16809CA010387E9C6CC", ksk[1], sizeof(ksk[1]));
	hex_to_bin("3ADCE3A3D3FBC977DD4B300A74749F13A3B04A5D73A2CD75A994E3195EFEBDAC"
		"6FF19B1F18D64851D5C74845C6407F0BF596A52E385E020127E83E54CFF5AC19", ksk[2], sizeof(ksk[2]));
	hex_to_bin("8400962BB769F63868CAE5A3FEC8DB6A9C8D3F1C846C8DCEEB642B6946EFA8E3"
		"98BE21001993A7EB1A1277FF74C15504183D25FDFCC05F0D4DEA892F6E301890", ksk[3], sizeof(ksk[3]));

	hex_to_bin("04266C033B91C1322CEB3446C901FFCF3CC40C4034E887C9597CA1893BA7330B"
	...
	hex_to_bin("57B9D2A711207F837421BAE7DD48EAA18EAB1A9A70A0F1305806FEE17B458F3A"
	...
	hex_to_bin("7DDA89F85B40539F5AD8C6DE4953F7094A715B63DDA30EC7CF65A785CEAE5FC6"
	...
	hex_to_bin("0AD71B0025F3D9A50DB338414D6D670E7799B7270A8444F6AE7F12AE7EB71BD0"
	...
	{
		print_safe("Failure! qsctest_ecdsa_kat_test: the public key generation does not match -EK1 \n");
		ret = false;
	}

	if (qsc_intutils_are_equal8(ksk[4], gsk, sizeof(ksk[4])) != true)
	{
		print_safe("Failure! qsctest_ecdsa_kat_test: the private key generation does not match -EK2 \n");
		ret = false;
	}
	...
	{
		print_safe("Failure! qsctest_ecdsa_kat_test: the signature does not match -EK3 \n");
		ret = false;
	}

	if (qsc_ecdsa_verify(msg0, &mlen, ksg0, sizeof(ksg0), kpk[0]) != true)
	{
		print_safe("Failure! qsctest_ecdsa_kat_test: signature verification has failed -EK4 \n");
		ret = false;
	}
	...
	{
		print_safe("Failure! qsctest_ecdsa_kat_test: the signature does not match -EK5 \n");
		ret = false;
	}

	if (qsc_ecdsa_verify(msg1, &mlen, ksg1, sizeof(ksg1), kpk[1]) != true)
	{
		print_safe("Failure! qsctest_ecdsa_kat_test: signature verification has failed -EK6 \n");
		ret = false;
	}
	...
	{
		print_safe("Failure! qsctest_ecdsa_kat_test: the signature does not match -EK7 \n");
		ret = false;
	}

	if (qsc_ecdsa_verify(msg2, &mlen, ksg2, sizeof(ksg2), kpk[2]) != true)
	{
		print_safe("Failure! qsctest_ecdsa_kat_test: signature verification has failed -EK8 \n");
		ret = false;
	}
	...
	{
		print_safe("Failure! qsctest_ecdsa_kat_test: the signature does not match -EK9 \n");
		ret = false;
	}

	if (qsc_ecdsa_verify(msg3, &mlen, ksg3, sizeof(ksg3), kpk[3]) != true)
	{
		print_safe("Failure! qsctest_ecdsa_kat_test: signature verification has failed -EK10 \n");
		ret = false;
	}
...
bool qsctest_ecdsa_privatekey_integrity()
{
	...
	{
		print_safe("Failure! qsctest_ecdsa_privatekey_integrity: ECDSA message verification passed with altered secret key -ES1 \n");
		ret = false;
	}
...
bool qsctest_ecdsa_publickey_integrity()
{
	...
	if (qsc_ecdsa_verify(mout, &msglen, sig, siglen, pk) == true)
	{
		print_safe("Failure! qsctest_ecdsa_publickey_integrity: ECDSA message verification passed with altered public key -EP1 \n");
		ret = false;
	}
...
bool qsctest_ecdsa_signature_integrity()
{
	...
	if (qsc_ecdsa_verify(mout, &msglen, sig, siglen, pk) == true)
	{
		print_safe("Failure! qsctest_ecdsa_signature_integrity: ECDSA message verification passed with altered secret key -EI1 \n");
		ret = false;
	}
	...
	for (i = 0; i < (int32_t)(siglen - QSCTEST_ECDSA_MSG0_SIZE); ++i)
	{
		...
		if (qsc_ecdsa_verify(mout, &msglen, sig, siglen, pk) == true)
		{
			print_safe("Failure! qsctest_ecdsa_signature_integrity: ECDSA flipping bit did not invalidate signature -EI2 \n");
			sig[i] ^= 1U;
			ret = false;
			break;
		}
...
bool qsctest_ecdsa_stress_test()
{
	...
	for (i = 0; i < QSCTEST_ECDSA_ITERATIONS; i++)
	{
		...
		{
			print_safe("Failure! qsctest_ecdsa_stress_test: ECDSA signature length is incorrect -ES1 \n");
			ret = false;
			break;
		}
		...
		if (qsc_ecdsa_verify(mout, &msglen, sig, siglen, pk) != true)
		{
			print_safe("Failure! qsctest_ecdsa_stress_test: ECDSA message verification has failed -ES2 \n");
			ret = false;
			break;
		}
		...
		{
			print_safe("Failure! qsctest_ecdsa_stress_test: ECDSA message length is incorrect -ES3 \n");
			ret = false;
			break;
		}
	}
...
void qsctest_ecdsa_run()
{
	...
	{
		print_safe("Success! Passed ECDSA known answer tests. \n");
	}
	else
	{
		print_safe("Failure! Failed ECDSA known answer test. \n");
	}

	if (qsctest_ecdsa_stress_test() == true)
	{
		print_safe("Success: The ECDSA stress test has succeeded. \n");
	}
	else
	{
		print_safe("Failure! The ECDSA stress test has failed. \n");
	}

	if (qsctest_ecdsa_publickey_integrity() == true)
	{
		print_safe("Success! The ECDSA altered public-key test has succeeded. \n");
	}
	else
	{
		print_safe("Failure! The ECDSA altered public-key test has failed. \n");
	}

	if (qsctest_ecdsa_privatekey_integrity() == true)
	{
		print_safe("Success! The ECDSA altered secret-key test has succeeded. \n");
	}
	else
	{
		print_safe("Failure! The ECDSA altered secret-key test has failed. \n");
	}

	if (qsctest_ecdsa_signature_integrity() == true)
	{
		print_safe("Success! The ECDSA altered signature test has succeeded. \n");
	}
	else
	{
		print_safe("Failure! The ECDSA altered signature test has failed. \n");
	}
}
