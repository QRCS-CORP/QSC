...
int32_t sphincsplus_generate(uint8_t* pk, uint8_t* sk)
{
	...
	ret = randombytes(seed, SPHINCSPLUS_SEED_SIZE);

	if (ret == QCX_STATUS_SUCCESS)
	{
		ret = sphincsplus_sign_seed_keypair(pk, sk, seed);
	}
...
int32_t sphincsplus_sign(uint8_t* sm, size_t* smlen, const uint8_t* m, size_t mlen, const uint8_t* sk)
{
	...
	ret = sphincsplus_sign_signature(sm, &siglen, m, (size_t)mlen, sk);
...
int32_t sphincsplus_verify(uint8_t* m, size_t* mlen, const uint8_t* sm, size_t smlen, const uint8_t* pk)
{
	...
	if (ret == QCX_STATUS_SUCCESS)
	{
		*mlen = smlen - SPX_BYTES;
		...
		if (sphincsplus_sign_verify(sm, SPX_BYTES, sm + SPX_BYTES, *mlen, pk) != QCX_STATUS_SUCCESS)
		{
			memset(m, 0, smlen);
			*mlen = 0;
			ret = QCX_ERROR_AUTHENTICATION;
		}
