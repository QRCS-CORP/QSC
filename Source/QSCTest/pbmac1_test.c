...
#define QSCTEST_PBMAC1_RFC9879_A1_DER_SIZE 2702U
#define QSCTEST_PBMAC1_RFC9879_A3_DER_SIZE 2736U
#define QSCTEST_PBMAC1_RFC9879_AUTHSAFE_OFFSET 30U
#define QSCTEST_PBMAC1_RFC9879_AUTHSAFE_SIZE 2546U
#define QSCTEST_PBMAC1_RFC9879_A1_SALT_OFFSET 2612U
#define QSCTEST_PBMAC1_RFC9879_A1_MAC_OFFSET 2657U
#define QSCTEST_PBMAC1_RFC9879_A3_SALT_OFFSET 2614U
#define QSCTEST_PBMAC1_RFC9879_A3_MAC_OFFSET 2659U
#define QSCTEST_PBMAC1_RFC9879_ITERATIONS 2048ULL

/* RFC 9879 Appendix A.1: Valid PKCS #12 File with SHA-256 HMAC and PRF. */
static const char QSCTEST_PBMAC1_RFC9879_A1_B64[] =
...
static const char QSCTEST_PBMAC1_RFC9879_A3_B64[] =
...
static bool pbmac1_decode_vector(uint8_t* output, size_t outlen, const char* vector)
{
    size_t declen;
    bool res;
...
static bool pbmac1_rfc9879_vector_test(const char* vector, size_t vectorlen, size_t saltoffset, size_t macoffset,
    qsc_pbmac1_hash_type hash, const char* keyhex, size_t maclen, const char* label)
{
    uint8_t der[QSCTEST_PBMAC1_RFC9879_A3_DER_SIZE] = { 0U };
    uint8_t expkey[QSC_PBMAC1_MAX_KEY_SIZE] = { 0U };
    uint8_t key[QSC_PBMAC1_MAX_KEY_SIZE] = { 0U };
    uint8_t mac[QSC_PBMAC1_MAX_MAC_SIZE] = { 0U };
    uint8_t smac[QSC_PBMAC1_MAX_MAC_SIZE] = { 0U };
    uint8_t bad[QSC_PBMAC1_MAX_MAC_SIZE] = { 0U };
    qsc_pbmac1_keyparams kp = { 0 };
    qsc_pbmac1_state state = { 0 };
    const uint8_t password[] = { 0x31U, 0x32U, 0x33U, 0x34U };
    size_t part0;
    size_t part1;
    bool status;
    ...
    if (pbmac1_decode_vector(der, vectorlen, vector) != true)
    {
        qsctest_print_safe("Failure! pbmac1_kat: could not decode RFC 9879 vector -");
        qsctest_print_safe(label);
        qsctest_print_safe(" \n");
        return false;
    }

    qsctest_hex_to_bin(keyhex, expkey, maclen);

    kp.password = password;
    kp.passwordlen = sizeof(password);
    kp.salt = der + saltoffset;
    kp.saltlen = 8U;
    kp.iterations = QSCTEST_PBMAC1_RFC9879_ITERATIONS;
    kp.hash = hash;
    kp.keylen = maclen;

    if (qsc_pbmac1_derive_key(key, maclen, &kp) != true || qsc_intutils_are_equal8(key, expkey, maclen) != true)
    {
        qsctest_print_safe("Failure! pbmac1_kat: derived key does not match the RFC 9879 vector -");
        qsctest_print_safe(label);
        qsctest_print_safe(" \n");
        status = false;
    }

    if (qsc_pbmac1_compute(mac, &kp, der + QSCTEST_PBMAC1_RFC9879_AUTHSAFE_OFFSET, QSCTEST_PBMAC1_RFC9879_AUTHSAFE_SIZE) != true ||
        qsc_intutils_are_equal8(mac, der + macoffset, maclen) != true)
    {
        qsctest_print_safe("Failure! pbmac1_kat: output does not match the RFC 9879 vector -");
        qsctest_print_safe(label);
        qsctest_print_safe(" \n");
        status = false;
    }

    if (qsc_pbmac1_verify(der + macoffset, maclen, &kp, der + QSCTEST_PBMAC1_RFC9879_AUTHSAFE_OFFSET,
        QSCTEST_PBMAC1_RFC9879_AUTHSAFE_SIZE) != true)
    {
        qsctest_print_safe("Failure! pbmac1_kat: verification failed on the RFC 9879 vector -");
        qsctest_print_safe(label);
        qsctest_print_safe(" \n");
        status = false;
    }

    qsc_memutils_copy(bad, der + macoffset, maclen);
    bad[0U] ^= 0x01U;

    if (qsc_pbmac1_verify(bad, maclen, &kp, der + QSCTEST_PBMAC1_RFC9879_AUTHSAFE_OFFSET,
        QSCTEST_PBMAC1_RFC9879_AUTHSAFE_SIZE) != false)
    {
        qsctest_print_safe("Failure! pbmac1_kat: modified MAC was accepted -");
        qsctest_print_safe(label);
        qsctest_print_safe(" \n");
        status = false;
    }
    ...
    if (qsc_pbmac1_initialize(&state, &kp) != true ||
        qsc_pbmac1_update(&state, der + QSCTEST_PBMAC1_RFC9879_AUTHSAFE_OFFSET, part0) != true ||
        qsc_pbmac1_update(&state, der + QSCTEST_PBMAC1_RFC9879_AUTHSAFE_OFFSET + part0, part1) != true ||
        qsc_pbmac1_update(&state, der + QSCTEST_PBMAC1_RFC9879_AUTHSAFE_OFFSET + part0 + part1,
            QSCTEST_PBMAC1_RFC9879_AUTHSAFE_SIZE - part0 - part1) != true ||
        qsc_pbmac1_finalize(&state, smac) != true ||
        qsc_intutils_are_equal8(smac, der + macoffset, maclen) != true)
    {
        qsctest_print_safe("Failure! pbmac1_kat: streaming output does not match the RFC 9879 vector -");
        qsctest_print_safe(label);
        qsctest_print_safe(" \n");
        status = false;
    }
    ...
    qsc_memutils_clear(expkey, sizeof(expkey));
    qsc_memutils_clear(key, sizeof(key));
    qsc_memutils_clear(mac, sizeof(mac));
    qsc_memutils_clear(smac, sizeof(smac));
...
{
    return pbmac1_rfc9879_vector_test(QSCTEST_PBMAC1_RFC9879_A1_B64,
        QSCTEST_PBMAC1_RFC9879_A1_DER_SIZE,
        QSCTEST_PBMAC1_RFC9879_A1_SALT_OFFSET,
        QSCTEST_PBMAC1_RFC9879_A1_MAC_OFFSET,
        qsc_pbmac1_hash_sha256,
        "823681901EBE0899CF7E99325ED45D318141C9FC0CBB93238DC94C321C49A119",
        QSC_PBMAC1_256_MAC_SIZE,
        "A1");
}

bool qsctest_pbmac1_512_kat(void)
{
    return pbmac1_rfc9879_vector_test(QSCTEST_PBMAC1_RFC9879_A3_B64,
        QSCTEST_PBMAC1_RFC9879_A3_DER_SIZE,
        QSCTEST_PBMAC1_RFC9879_A3_SALT_OFFSET,
        QSCTEST_PBMAC1_RFC9879_A3_MAC_OFFSET,
        qsc_pbmac1_hash_sha512,
        "65857E58F0853B3ABE2222A86FABCD92A71F844BB12A0AB61162F0453C84C1BAED1D5E293813EC91CB75A4B6B07FCD4564CB0439C910DF94A083326576F0F1F1",
        QSC_PBMAC1_512_MAC_SIZE,
        "A3");
}

void qsctest_pbmac1_run(void)
{
    if (qsctest_pbmac1_256_kat() == true)
    {
        qsctest_print_safe("Success! Passed the PBMAC1-HMAC(SHA2-256) RFC 9879 KAT test. \n");
    }
    else
    {
        qsctest_print_safe("Failure! Failed the PBMAC1-HMAC(SHA2-256) RFC 9879 KAT test. \n");
    }

    if (qsctest_pbmac1_512_kat() == true)
    {
        qsctest_print_safe("Success! Passed the PBMAC1-HMAC(SHA2-512) RFC 9879 KAT test. \n");
    }
    else
    {
        qsctest_print_safe("Failure! Failed the PBMAC1-HMAC(SHA2-512) RFC 9879 KAT test. \n");
    }
}
