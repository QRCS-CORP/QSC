...
static bool aesavs_cbc128_kat(const char* filepath)
{
	...
	QSC_SIMD_ALIGN uint8_t exp[QSC_AES_BLOCK_SIZE] = { 0 };
	...
	QSC_SIMD_ALIGN uint8_t otp[QSC_AES_BLOCK_SIZE] = { 0 };
	QSC_SIMD_ALIGN uint8_t pln[QSC_AES_BLOCK_SIZE] = { 0 };
	int64_t read;
	size_t i;
	size_t len;
	errno_t err;
		...
		while (read != -1)
		{
			...
			if (read > 0 && line != NULL)
			{
				if (memcmp(line, CBC_COUNT, strlen(CBC_COUNT)) == 0)
				{
					...
					if (qsc_intutils_are_equal8(exp, otp, QSC_AES_BLOCK_SIZE) == false)
					{
						res = false;
						break;
					}
					...
					if (qsc_intutils_are_equal8(pln, dec, QSC_AES_BLOCK_SIZE) == false)
					{
						res = false;
						break;
					}
...
static bool aesavs_cbc128_mct(const char* filepath)
{
	...
	const char* CBC_CIPHERTEXT = "CIPHERTEXT = ";
	char* line;
	QSC_SIMD_ALIGN uint8_t enc[QSC_AES_BLOCK_SIZE] = { 0 };
	QSC_SIMD_ALIGN uint8_t exp[QSC_AES_BLOCK_SIZE] = { 0 };
	QSC_SIMD_ALIGN uint8_t iv[QSC_AES_BLOCK_SIZE] = { 0 };
	QSC_SIMD_ALIGN uint8_t ivc[QSC_AES_BLOCK_SIZE] = { 0 };
	QSC_SIMD_ALIGN uint8_t key[QSC_AES128_KEY_SIZE] = { 0 };
	QSC_SIMD_ALIGN uint8_t otp[QSC_AES_BLOCK_SIZE] = { 0 };
	QSC_SIMD_ALIGN uint8_t otpc[QSC_AES_BLOCK_SIZE] = { 0 };
	QSC_SIMD_ALIGN uint8_t pln[QSC_AES_BLOCK_SIZE] = { 0 };
	QSC_SIMD_ALIGN uint8_t plnc[QSC_AES_BLOCK_SIZE] = { 0 };
	int64_t read;
	size_t count;
	size_t i;
	size_t len;
			...
			if (read > 0 && line != NULL)
			{
				if (memcmp(line, CBC_COUNT, strlen(CBC_COUNT)) == 0)
				{
					...
					else
					{
						...
						if (qsc_intutils_are_equal8(otp, pln, QSC_AES_BLOCK_SIZE) == false)
						{
							res = false;
							break;
						}
