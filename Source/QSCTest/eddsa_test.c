...
bool qsctest_eddsa_kat_test()
{
	...
	uint8_t kms0[QSCTEST_EDDSA_MSG0_SIZE] = { 0 };
	uint8_t kms1[QSCTEST_EDDSA_MSG1_SIZE] = { 0 };
	uint8_t kms2[QSCTEST_EDDSA_MSG2_SIZE] = { 0 };
	uint8_t kms3[QSCTEST_EDDSA_MSG3_SIZE] = { 0 };
	uint8_t ksg0[QSCTEST_EDDSA_MSG0_SIZE + QSC_ECDSA_SIGNATURE_SIZE] = { 0 };
	uint8_t ksg1[QSCTEST_EDDSA_MSG1_SIZE + QSC_ECDSA_SIGNATURE_SIZE] = { 0 };
	uint8_t ksg2[QSCTEST_EDDSA_MSG2_SIZE + QSC_ECDSA_SIGNATURE_SIZE] = { 0 };
	uint8_t ksg3[QSCTEST_EDDSA_MSG3_SIZE + QSC_ECDSA_SIGNATURE_SIZE] = { 0 };
	uint8_t msg0[QSCTEST_EDDSA_MSG0_SIZE] = { 0 };
	uint8_t msg1[QSCTEST_EDDSA_MSG1_SIZE] = { 0 };
	uint8_t msg2[QSCTEST_EDDSA_MSG2_SIZE] = { 0 };
	uint8_t msg3[QSCTEST_EDDSA_MSG3_SIZE] = { 0 };
	uint8_t sig0[QSCTEST_EDDSA_MSG0_SIZE + QSC_ECDSA_SIGNATURE_SIZE] = { 0 };
	uint8_t sig1[QSCTEST_EDDSA_MSG1_SIZE + QSC_ECDSA_SIGNATURE_SIZE] = { 0 };
	uint8_t sig2[QSCTEST_EDDSA_MSG2_SIZE + QSC_ECDSA_SIGNATURE_SIZE] = { 0 };
	uint8_t sig3[QSCTEST_EDDSA_MSG3_SIZE + QSC_ECDSA_SIGNATURE_SIZE] = { 0 };
	size_t mlen;
	size_t slen;
	bool ret;
	...
	/* 32-byte message */
	qsc_eddsa_sign(sig0, &slen, kms0, sizeof(kms0), ksk[0]);
	...
	if (qsc_eddsa_verify(msg0, &mlen, ksg0, sizeof(ksg0), kpk[0]) != true)
	{
	...
	/* 64-byte message */
	qsc_eddsa_sign(sig1, &slen, kms1, sizeof(kms1), ksk[1]);

	if (qsc_intutils_are_equal8(ksg1, sig1, sizeof(ksg1)) != true)
	{
		qsctest_print_safe("Failure! qsctest_eddsa_kat_test: the signature does not match -EK5 \n");
		ret = false;
	}

	if (qsc_eddsa_verify(msg1, &mlen, ksg1, sizeof(ksg1), kpk[1]) != true)
	{
		qsctest_print_safe("Failure! qsctest_eddsa_kat_test: signature verification has failed -EK6 \n");
		ret = false;
	}

	/* 96-byte message */
	qsc_eddsa_sign(sig2, &slen, kms2, sizeof(kms2), ksk[2]);

	if (qsc_intutils_are_equal8(ksg2, sig2, sizeof(ksg2)) != true)
	{
		qsctest_print_safe("Failure! qsctest_eddsa_kat_test: the signature does not match -EK7 \n");
		ret = false;
	}

	if (qsc_eddsa_verify(msg2, &mlen, ksg2, sizeof(ksg2), kpk[2]) != true)
	{
		qsctest_print_safe("Failure! qsctest_eddsa_kat_test: signature verification has failed -EK8 \n");
		ret = false;
	}

	/* 128-byte message */
	qsc_eddsa_sign(sig3, &slen, kms3, sizeof(kms3), ksk[3]);

	if (qsc_intutils_are_equal8(ksg3, sig3, sizeof(ksg3)) != true)
	{
		qsctest_print_safe("Failure! qsctest_eddsa_kat_test: the signature does not match -EK9 \n");
		ret = false;
	}

	if (qsc_eddsa_verify(msg3, &mlen, ksg3, sizeof(ksg3), kpk[3]) != true)
	{
		qsctest_print_safe("Failure! qsctest_eddsa_kat_test: signature verification has failed -EK10 \n");
		ret = false;
	}
...
bool qsctest_eddsa_privatekey_integrity()
{
	...
	uint8_t mout[QSC_ECDSA_SIGNATURE_SIZE + QSCTEST_EDDSA_MSG0_SIZE] = { 0 };
	uint8_t seed[QSCTEST_NIST_RNG_SEED_SIZE] = { 0 };
	uint8_t sig[QSC_ECDSA_SIGNATURE_SIZE + QSCTEST_EDDSA_MSG0_SIZE] = { 0 };
...
bool qsctest_eddsa_signature_integrity()
{
	...
	for (size_t i = 0; i < (int32_t)(siglen - QSCTEST_EDDSA_MSG0_SIZE); ++i)
	{
		sig[i] ^= 1U;
...
bool qsctest_eddsa_stress_test()
{
	...
	siglen = QSC_ECDSA_SIGNATURE_SIZE + QSCTEST_EDDSA_MSG0_SIZE; 
	qsctest_hex_to_bin("061550234D158C5EC95595FE04EF7A25767F2E24CC2BC479D09D86DC9ABCFDE7056A8C266F9EF97ED08541DBD2E1FFA1", seed, sizeof(seed));
	qsctest_nistrng_prng_initialize(seed, NULL, 0);
	...
	for (size_t i = 0; i < QSCTEST_EDDSA_ITERATIONS; i++)
	{
		...
		if (siglen != QSC_ECDSA_SIGNATURE_SIZE + QSCTEST_EDDSA_MSG0_SIZE)
		{
