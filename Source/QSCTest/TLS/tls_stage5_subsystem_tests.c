...
static bool qsctest_tls_stage5_stub_validate(const qsc_tls_certificate_view* chain, size_t chainlength, const qsc_tls_certificate_validation_context* ctx, void* state)
{
	...
	return (chainlength == 1U && chain[0].datalen == 32U);
}

static bool qsctest_tls_stage5_stub_verify(qsc_tls_signature_scheme scheme, const uint8_t* input, size_t inputlen,
...
static bool qsctest_tls_stage5_app_data_roundtrip(void)
{
	...
	qsc_tls_connection c = { 0 };
	qsc_tls_connection s = { 0 };
	...
	res = (qsctest_tls_stage5_do_handshake(&c, &s) == 1);

	if (res == true)
	{
		reclen = 0U;
		st = qsc_tls_engine_write_application_data(&c, payload_c, sizeof(payload_c) - 1U, recbuf, sizeof(recbuf), &reclen);
	...
	if (res == true)
	{
		consumed = 0U;
		plainlen = 0U;
		st = qsc_tls_engine_read_application_data(&s, recbuf, reclen, &consumed, plain, sizeof(plain), &plainlen);
	...
	{
		reclen = 0U;
		st = qsc_tls_engine_write_application_data(&s, payload_s, sizeof(payload_s) - 1U, recbuf, sizeof(recbuf), &reclen);
		res = (st == qsc_tls_status_success && reclen > 0U);
	}
	...
	if (res == true)
	{
		consumed = 0U;
		plainlen = 0U;
		st = qsc_tls_engine_read_application_data(&c, recbuf, reclen, &consumed, plain, sizeof(plain), &plainlen);
		res = (st == qsc_tls_status_success);
	}
	...
	{
		reclen = 0U;
		st = qsc_tls_engine_close(&c, recbuf, sizeof(recbuf), &reclen);
		res = (st == qsc_tls_status_success && reclen > 0U);
	}
...
static bool qsctest_tls_stage5_malformed_client_hello(void)
{
	...
	uint8_t mal[9U] = { 0x16U, 0x03U, 0x03U, 0x00U, 0x04U, 0x01U, 0xFFU, 0xFFU, 0xFFU };
	uint8_t out[256U];
	size_t consumed;
	size_t written;
	qsc_tls_status st;
...
static bool qsctest_tls_stage5_tampered_finished(void)
{
	...
	c2slen = 0U;
	s2c_len = 0U;
	st = qsc_tls_engine_handshake(&c, NULL, 0U, &consumed, bufc2s, sizeof(bufc2s), &c2slen);
	res = (st == qsc_tls_status_success && c2slen > 0U);

	if (res == true)
	{
		st = qsc_tls_engine_handshake(&s, bufc2s, c2slen, &consumed, bufs2c, sizeof(bufs2c), &s2c_len);
	...
	while (res == true && offset < s2c_len && qsc_tls_engine_is_handshake_complete(&c) == false)
	{
		size_t w;

		w = 0U;
		st = qsc_tls_engine_handshake(&c, bufs2c + offset, s2c_len - offset, &consumed, bufc2s, sizeof(bufc2s), &w);
		res = (st == qsc_tls_status_success);
		offset += consumed;
	...
	if (res == true)
	{
		res = (cfinlen > 0U && qsc_tls_engine_is_handshake_complete(&c) == true);
	}
	...
	if (res == true)
	{
		st = qsc_tls_engine_handshake(&s, bufc2s, cfinlen, &consumed, junk, sizeof(junk), &written);
		res = (st != qsc_tls_status_success);
	}
