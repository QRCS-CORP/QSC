...
static bool qsctest_tls_stage3_stub_validate(const qsc_tls_certificate_view* chain, size_t chainlength, 
...
static bool qsctest_tls_stage3_keyupdate_e2e(void)
{
	...
	qsc_eddsa_generate_keypair(qsctest_tls_stage3_server_pk, qsctest_tls_stage3_server_sk, qsc_secrand_generate);
	...
	res = qsctest_tls_stage3_do_handshake(&c, &s);

	if (res == true)
	{
		res = qsctest_tls_stage3_send_app(&c, &s, msg1, sizeof(msg1));
	}

	if (res == true)
	{
		res = qsctest_tls_stage3_send_app(&s, &c, r1, sizeof(r1));
	}

	if (res == true)
	{
		kulen = 0U;
		st = qsc_tls_engine_request_key_update(&c, false, kurec, sizeof(kurec), &kulen);
	...
	if (res == true)
	{
		...
		st = qsc_tls_engine_read_application_data_ex(&s, kurec, kulen, &rxconsumed,
			rxplain, sizeof(rxplain), &rxplainlen, rxresp, sizeof(rxresp), &rxresplen);
	...
	if (res == true)
	{
		res = qsctest_tls_stage3_send_app(&c, &s, msg2, sizeof(msg2));
	}

	if (res == true)
	{
		res = qsctest_tls_stage3_send_app(&s, &c, r2, sizeof(r2));
	}

	if (res == true)
	{
		kulen = 0U;
		st = qsc_tls_engine_request_key_update(&s, true, kurec, sizeof(kurec), &kulen);
		res = (st == qsc_tls_status_success && kulen > 0U);
	}
	...
	if (res == true)
	{
		...
		st = qsc_tls_engine_read_application_data_ex(&c, kurec, kulen, &rxconsumed,
			rxplain, sizeof(rxplain), &rxplainlen, rxresp, sizeof(rxresp), &rxresplen);
	...
	if (res == true)
	{
		...
		st = qsc_tls_engine_read_application_data_ex(&s, rxresp, rxresplen, &rxconsumed,
			rxplain, sizeof(rxplain), &rxplainlen, NULL, 0U, &dummy);
	...
	if (res == true)
	{
		res = qsctest_tls_stage3_send_app(&c, &s, msg3, sizeof(msg3));
	}

	if (res == true)
	{
		res = qsctest_tls_stage3_send_app(&s, &c, r3, sizeof(r3));
	}
