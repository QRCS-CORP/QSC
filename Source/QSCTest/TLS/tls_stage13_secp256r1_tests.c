...
static bool qsctest_tls_stage13_secp256r1_roundtrip(void)
{
	...
	else
	{
		...
		{
			st = qsc_tls_groups_server_respond(qsc_tls_group_secp256r1,
				client.publicshare,
				client.publicsharelen,
				serverpub,
				sizeof(serverpub),
				&serverpublen,
				serverss,
			...
			if (st == qsc_tls_status_success &&
				...
				serverpub[0U] == 0x04U)
			{
				st = qsc_tls_groups_client_derive_shared_secret(&client,
					serverpub,
					serverpublen,
					clientss,
					sizeof(clientss),
				...
				if (st == qsc_tls_status_success && clientsslen == serversslen &&
					qsc_memutils_are_equal(clientss, serverss, clientsslen) == true)
				{
					res = true;
				}
			}
