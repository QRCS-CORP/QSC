 ...
 *
 * The state machine implements TLS 1.3 full handshakes, HelloRetryRequest,
 * certificate-based mutual authentication, and 1-RTT PSK-DHE resumption. QSC
 * deliberately does not implement 0-RTT application data; attempts to enable it
...
typedef struct qsc_tls_client_config
{
    ...
    qsc_tls_local_certificate_config localcert;               /*!< Optional client certificate chain and signer used for mutual TLS authentication. */
    qsc_tls_alpn_protocols alpn;                              /*!< Configured ALPN client protocol list and policy. */
    const qsc_tls_session_ticket* offeredticket;              /*!< Optional PSK ticket to offer for resumption; NULL = fresh handshake. */
    bool enableresumption;                                    /*!< Advertise support for TLS 1.3 PSK resumption and accept NewSessionTicket messages. */
    bool enableearlydata;                                     /*!< Reserved 0-RTT switch; QSC rejects true because application early data is not supported. */
...
typedef struct qsc_tls_client_state
{
    ...
    uint8_t clienthello[QSC_TLS_CLIENT_HELLO_BODY_MAX_SIZE + 4U]; /*!< Serialized first ClientHello handshake message. */
    uint8_t retryclienthello[QSC_TLS_CLIENT_HELLO_BODY_MAX_SIZE + 4U]; /*!< Serialized second ClientHello handshake message after HelloRetryRequest. */
    uint8_t helloretryrequest[QSC_TLS_SERVER_HELLO_BODY_MAX_SIZE + 4U]; /*!< Serialized HelloRetryRequest handshake message. */
    ...
    uint8_t selectedalpn[QSC_TLS_MAX_ALPN_SIZE];              /*!< Selected ALPN protocol identifier without a terminating NULL byte. */
    uint8_t peercertificate[QSC_TLS_CERTIFICATE_MAX_SIZE];     /*!< DER encoded peer leaf certificate retained for CertificateVerify validation. */
    qsc_tls_signature_scheme clientauthsigschemes[QSC_TLS_MAX_SIGNATURE_SCHEMES]; /*!< Signature schemes accepted by the server for client CertificateVerify. */
    size_t selectedalpnlen;                                   /*!< Length, in bytes, of selectedalpn. */
    size_t peercertificatelen;                                /*!< Length, in bytes, of the retained peer leaf certificate. */
    size_t clientauthsigschemecount;                          /*!< Number of valid entries in clientauthsigschemes. */
    size_t clienthellolen;                                    /*!< Length, in bytes, of the retained first ClientHello. */
    size_t retryclienthellolen;                               /*!< Length, in bytes, of the retained second ClientHello. */
    size_t helloretryrequestlen;                              /*!< Length, in bytes, of the retained HelloRetryRequest. */
    bool alpnselected;                                        /*!< True when the server selected an ALPN protocol. */
    bool servercertificatevalidationattempted;                /*!< True after the server certificate chain has been submitted to the configured validation interface. */
    bool serverauthenticated;                                 /*!< True once the server certificate path and CertificateVerify have been validated. */
    bool clientauthrequested;                                 /*!< True when the server sent a valid main-handshake CertificateRequest. */
    bool clientcertificatesent;                               /*!< True when a non-empty client Certificate was sent in response to CertificateRequest. */
    ...
    bool clientrandomgenerated;                               /*!< True once the ClientHello random has been generated for this handshake. */
    ...
    bool earlydataoffered;                                    /*!< Reserved compatibility field; always false because QSC does not emit 0-RTT early_data. */
...
/**
 * \brief Configure a local client certificate chain and private signing key.
 *
 * \details
 * The configured identity is used only when the server sends a main-handshake
 * CertificateRequest whose signature_algorithms extension permits \p verifyscheme.
 * If no compatible identity is configured, the client sends the required empty
 * Certificate message and continues without certificate authentication.
 *
 * \param config: [struct*] The client configuration to update.
 * \param chain: [const struct*] Client certificate chain views in leaf-first order.
 * \param chainlength: [size_t] Number of entries in the certificate chain.
 * \param verifyscheme: [enum] Signature scheme used for client CertificateVerify.
 * \param privatekeydata: [const uint8_t*] Raw private signing key bytes.
 * \param privatekeylen: [size_t] Length of the private signing key in bytes.
 *
 * \return [qsc_tls_status] Returns qsc_tls_status_success on success.
 */
QSC_EXPORT_API qsc_tls_status qsc_tls_client_config_set_local_certificate(qsc_tls_client_config* config,
 ...
 *
 * \details
 * Consumes one complete inbound TLS record, advances the client handshake state
 * machine, and emits any required response records into \p output. Handshake
 * messages fragmented across records are retained and reassembled until the complete
 * message is available. Fragmented Handshake messages may not be interleaved with
 * another record content type or span a TLS 1.3 traffic-key transition.
 *
 * \param state: [struct*] The active client handshake state.
