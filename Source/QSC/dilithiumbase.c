...
static const QSC_CACHE_ALIGNED int32_t dilithium_zetas[DILITHIUM_N] =
{
...
{
    QSC_CACHE_ALIGNED uint8_t buf[DILITHIUM_POLY_UNIFORM_NBLOCKS * QSC_KECCAK_128_RATE + 2U];
    ...
    while (ctr < DILITHIUM_N)
    {
        ...
        qsc_keccak_squeezeblocks(&kctx, buf + off, 1U, QSC_KECCAK_128_RATE, QSC_KECCAK_PERMUTATION_ROUNDS);
...
{
    QSC_CACHE_ALIGNED uint8_t buf[QSC_KECCAK_256_RATE];
    qsc_keccak_state kctx;
    uint64_t signs;
    size_t i;
    size_t b;
...
static void dilithium_polyeta_pack(uint8_t* r, const dilithium_poly* a)
{
    ...
    {
        t[0U] = (uint8_t)(DILITHIUM_ETA - a->coeffs[8U * i]);
        t[1U] = (uint8_t)(DILITHIUM_ETA - a->coeffs[(8U * i) + 1U]);
        t[2U] = (uint8_t)(DILITHIUM_ETA - a->coeffs[(8U * i) + 2U]);
        t[3U] = (uint8_t)(DILITHIUM_ETA - a->coeffs[(8U * i) + 3U]);
        t[4U] = (uint8_t)(DILITHIUM_ETA - a->coeffs[(8U * i) + 4U]);
        t[5U] = (uint8_t)(DILITHIUM_ETA - a->coeffs[(8U * i) + 5U]);
        t[6U] = (uint8_t)(DILITHIUM_ETA - a->coeffs[(8U * i) + 6U]);
        t[7U] = (uint8_t)(DILITHIUM_ETA - a->coeffs[(8U * i) + 7U]);

        r[3U * i] = (uint8_t)((t[0U] >> 0) | (t[1U] << 3) | (t[2U] << 6));
        r[(3U * i) + 1U] = (uint8_t)((t[2U] >> 2) | (t[3U] << 1) | (t[4U] << 4) | (t[5U] << 7));
        r[(3U * i) + 2U] = (uint8_t)((t[5U] >> 1) | (t[6U] << 2) | (t[7U] << 5));
    }
#elif DILITHIUM_ETA == 4
    for (size_t i = 0U; i < DILITHIUM_N / 2U; ++i)
    {
        t[0U] = (uint8_t)(DILITHIUM_ETA - a->coeffs[2U * i]);
        t[1U] = (uint8_t)(DILITHIUM_ETA - a->coeffs[(2U * i) + 1U]);
        r[i] = (uint8_t)(t[0U] | (t[1U] << 4));
    }
#endif
}
...
static void dilithium_polyt1_pack(uint8_t* r, const dilithium_poly* a)
{
    ...
    {
        r[5U * i] = (uint8_t)(a->coeffs[4U * i] >> 0);
...
static void dilithium_polyt1_unpack(dilithium_poly* r, const uint8_t* a)
{
    for (size_t i = 0U; i < DILITHIUM_N / 4U; ++i)
    {
        ...
        r->coeffs[(4U * i) + 1U] = ((a[(5 * i) + 1U] >> 2) | ((uint32_t)a[(5U * i) + 2U] << 6)) & 0x000003FF;
        r->coeffs[(4U * i) + 2U] = ((a[(5 * i) + 2U] >> 4) | ((uint32_t)a[(5U * i) + 3U] << 4)) & 0x000003FF;
        r->coeffs[(4U * i) + 3U] = ((a[(5 * i) + 3U] >> 6) | ((uint32_t)a[(5U * i) + 4U] << 2)) & 0x000003FF;
    }
}
...
static void dilithium_polyw1_pack(uint8_t* r, const dilithium_poly* a)
{
    ...
    for (size_t i = 0U; i < DILITHIUM_N / 4U; ++i)
    {
        r[3U * i] = (uint8_t)a->coeffs[4U * i];
        r[3U * i] |= (uint8_t)(a->coeffs[(4U * i) + 1U] << 6);
        r[(3U * i) + 1U] = (uint8_t)(a->coeffs[(4U * i) + 1U] >> 2);
        r[(3U * i) + 1U] |= (uint8_t)(a->coeffs[(4U * i) + 2U] << 4);
        r[(3U * i) + 2U] = (uint8_t)(a->coeffs[(4U * i) + 2U] >> 4);
        r[(3U * i) + 2U] |= (uint8_t)(a->coeffs[(4U * i) + 3U] << 2);
    }
#elif (DILITHIUM_GAMMA2 == (DILITHIUM_Q - 1) / 32)
    for (size_t i = 0U; i < DILITHIUM_N / 2U; ++i)
    {
        r[i] = (uint8_t)(a->coeffs[2U * i] | (a->coeffs[(2U * i) + 1U] << 4));
    }
#endif
}
...
{
    QSC_CACHE_ALIGNED uint8_t buf[DILITHIUM_POLY_UNIFORM_ETA_NBLOCKS * QSC_KECCAK_256_RATE];
    qsc_keccak_state kctx;
    size_t ctr;
    size_t buflen;
...
{
    QSC_CACHE_ALIGNED uint8_t buf[DILITHIUM_POLY_UNIFORM_GAMMA1_NBLOCKS * QSC_KECCAK_256_RATE];
    qsc_keccak_state kctx;
...
static void dilithium_polyvec_matrix_expand(dilithium_polyvecl mat[DILITHIUM_K], const uint8_t rho[DILITHIUM_SEEDBYTES])
{
    ...
    {
        for (size_t j = 0U; j < DILITHIUM_L; ++j)
        {
...
static void dilithium_pack_pk(uint8_t pk[DILITHIUM_PUBLICKEY_SIZE], const uint8_t rho[DILITHIUM_SEEDBYTES], const dilithium_polyveck* t1)
{
    ...
    for (i = 0U; i < DILITHIUM_SEEDBYTES; ++i)
    {
        pk[i] = rho[i];
    }
...
static void dilithium_pack_sig(uint8_t sig[DILITHIUM_SIGNATURE_SIZE], const uint8_t c[DILITHIUM_CTILDEBYTES], const dilithium_polyvecl* z, const dilithium_polyveck* h)
{
    ...
    for (i = 0U; i < DILITHIUM_CTILDEBYTES; ++i)
    {
        sig[i] = c[i];
    }
...
bool qsc_dilithium_ref_generate_keypair(uint8_t* pk, uint8_t* sk, bool (*rng_generate)(uint8_t*, size_t))
{
    ...
    QSC_CACHE_ALIGNED uint8_t seedbuf[(2U * DILITHIUM_SEEDBYTES) + DILITHIUM_CRHBYTES];
    QSC_CACHE_ALIGNED uint8_t tr[DILITHIUM_TRBYTES];
    const uint8_t* rho;
    ...
    if (res)
    {
        ...
        dilithium_polyvecl_uniform_eta(&s1, rhoprime, 0U);
...
bool qsc_dilithium_ref_sign_signature(uint8_t* sig, size_t* siglen, const uint8_t* message, size_t msglen, const uint8_t* context, size_t ctxlen, const uint8_t* sk, bool (*rng_generate)(uint8_t*, size_t))
{
    QSC_CACHE_ALIGNED uint8_t seedbuf[(2U * DILITHIUM_SEEDBYTES) + DILITHIUM_TRBYTES + (2U * DILITHIUM_CRHBYTES)];
    QSC_CACHE_ALIGNED dilithium_polyvecl mat[DILITHIUM_K];
    dilithium_polyvecl s1;
...
bool qsc_dilithium_ref_verify(const uint8_t* sig, size_t siglen, const uint8_t* message, size_t msglen, const uint8_t* context, size_t ctxlen, const uint8_t* pk)
{
    QSC_CACHE_ALIGNED uint8_t buf[DILITHIUM_K * DILITHIUM_POLYW1_PACKEDBYTES];
    QSC_CACHE_ALIGNED uint8_t rho[DILITHIUM_SEEDBYTES];
    QSC_CACHE_ALIGNED uint8_t mu[DILITHIUM_CRHBYTES];
    QSC_CACHE_ALIGNED uint8_t c[DILITHIUM_CTILDEBYTES];
    QSC_CACHE_ALIGNED uint8_t c2[DILITHIUM_CTILDEBYTES];
    dilithium_polyvecl mat[DILITHIUM_K];
    dilithium_polyvecl z;
    dilithium_polyveck h;
    dilithium_polyveck t1;
...
bool qsc_dilithium_ref_open(uint8_t* message, size_t* msglen, const uint8_t* context, size_t ctxlen, const uint8_t* sm, size_t smlen, const uint8_t* pk)
{
    bool res;
    ...
    if (ctxlen <= 255U)
    {
        ...
        if (smlen >= DILITHIUM_SIGNATURE_SIZE)
        {
            *msglen = smlen - DILITHIUM_SIGNATURE_SIZE;
            res = qsc_dilithium_ref_verify(sm, DILITHIUM_SIGNATURE_SIZE, sm + DILITHIUM_SIGNATURE_SIZE, *msglen, prec, ctxlen + 2U, pk);
