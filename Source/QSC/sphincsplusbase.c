...
#define SPX_FULL_HEIGHT 66U
/* Number of subtree layer. */
#define SPX_D 22U
/* FORS tree dimensions. */
#define SPX_FORS_HEIGHT 6U
#define SPX_FORS_TREES 33U
...
#define SPX_N 32U
/* Height of the hypertree. */
#define SPX_FULL_HEIGHT 68U
/* Number of subtree layer. */
#define SPX_D 17U
/* FORS tree dimensions. */
#define SPX_FORS_HEIGHT 9U
#define SPX_FORS_TREES 35U
/* Winternitz parameter, */
#define SPX_WOTS_W 16U
...
#define SPX_D 16U
/* FORS tree dimensions. */
#define SPX_FORS_HEIGHT 10U
#define SPX_FORS_TREES 38U
/* Winternitz parameter, */
#define SPX_WOTS_W 16U
...
#if ((SPX_TREE_HEIGHT * (SPX_D - 1U)) > 64U)
...
static void sphincsplus_prf_addr(uint8_t* out, const uint8_t* key, const uint32_t addr[8U])
{
    /* Computes PRF(key, addr), given a secret key of SPX_N bytes and an address */

    QSC_SIMD_ALIGN uint8_t buf[SPX_N + SPX_ADDR_BYTES];
...
// inblock = x1, x2, SPX_FORS_TREES, SPX_WOTS_LEN


static void sphincsplus_thash_x1(uint8_t* out, const uint8_t* in, const uint8_t* pubseed, uint32_t addr[8U])
{
    /* Takes an array of 1 inblocks concatenated arrays of SPX_N bytes */
    const size_t BLKLEN = SPX_N;
    const size_t KEYLEN = SPX_N + SPX_ADDR_BYTES;

    QSC_SIMD_ALIGN uint8_t blkbuf[SPX_N + SPX_N + SPX_ADDR_BYTES] = { 0U };
    QSC_SIMD_ALIGN uint8_t bitmask[SPX_N];

    qsc_memutils_copy(blkbuf, pubseed, SPX_N);
    qsc_memutils_copy(blkbuf + SPX_N, (uint8_t*)addr, SPX_ADDR_BYTES);

#if defined(QSC_SPHINCSPLUS_EXTENDED)

#else
    qsc_shake256_compute(bitmask, BLKLEN, blkbuf, KEYLEN);
#endif

    for (size_t i = 0U; i < BLKLEN; ++i)
    {
        blkbuf[KEYLEN + i] = in[i] ^ bitmask[i];
    }

#if defined(QSC_SPHINCSPLUS_EXTENDED)
    qsc_shake512_compute(out, SPX_N, blkbuf, KEYLEN + BLKLEN);
#else
    qsc_shake256_compute(out, SPX_N, blkbuf, KEYLEN + BLKLEN);
#endif
}

static void sphincsplus_thash_x2(uint8_t* out, const uint8_t* in, const uint8_t* pubseed, uint32_t addr[8U])
{
    /* Takes an array of 1 inblocks concatenated arrays of SPX_N bytes */
    const size_t BLKLEN = 2U * SPX_N;
    const size_t KEYLEN = SPX_N + SPX_ADDR_BYTES;

    QSC_SIMD_ALIGN uint8_t blkbuf[(2U * SPX_N) + SPX_N + SPX_ADDR_BYTES] = { 0U };
    QSC_SIMD_ALIGN uint8_t bitmask[(2U * SPX_N)] = { 0U };
...
static void sphincsplus_thash_fors(uint8_t* out, const uint8_t* in, const uint8_t* pubseed, uint32_t addr[8U])
{
    /* Takes an array of 1 inblocks concatenated arrays of SPX_N bytes */
    const size_t BLKLEN = SPX_FORS_TREES * SPX_N;
    const size_t KEYLEN = SPX_N + SPX_ADDR_BYTES;

    QSC_SIMD_ALIGN uint8_t blkbuf[(SPX_FORS_TREES * SPX_N) + SPX_N + SPX_ADDR_BYTES] = { 0U };
    QSC_SIMD_ALIGN uint8_t bitmask[(SPX_FORS_TREES * SPX_N)] = { 0U };
...
static void sphincsplus_thash_wots(uint8_t* out, const uint8_t* in, const uint8_t* pubseed, uint32_t addr[8U])
{
    /* Takes an array of 1 inblocks concatenated arrays of SPX_N bytes */
    const size_t BLKLEN = SPX_WOTS_LEN * SPX_N;
    const size_t KEYLEN = SPX_N + SPX_ADDR_BYTES;

    QSC_SIMD_ALIGN uint8_t blkbuf[(SPX_WOTS_LEN * SPX_N) + SPX_N + SPX_ADDR_BYTES] = { 0U };
    QSC_SIMD_ALIGN uint8_t bitmask[(SPX_WOTS_LEN * SPX_N)] = { 0U };
...
static void sphincsplus_compute_root(uint8_t* root, const uint8_t* leaf, uint32_t leaf_idx, uint32_t idx_offset, const uint8_t* auth_path,
    uint32_t tree_height, const uint8_t* pubseed, uint32_t addr[8U])
{
    /* Computes a root node given a leaf and an auth path.
       Expects address to be complete other than the tree_height and tree_index */

    QSC_SIMD_ALIGN uint8_t buffer[2U * SPX_N] = { 0U };
    ...
    for (size_t i = 0U; i < tree_height - 1U; ++i)
    {
        leaf_idx >>= 1;
        idx_offset >>= 1;

        /* Set the address of the node we're creating. */
        sphincsplus_set_tree_height(addr, (uint32_t)i + 1U);
        sphincsplus_set_tree_index(addr, leaf_idx + idx_offset);

        /* Pick the right or left neighbor, depending on parity of the node. */
        if ((leaf_idx & 1) != 0U)
        {
            sphincsplus_thash_x2(buffer + SPX_N, buffer, pubseed, addr);
            qsc_memutils_copy(buffer, auth_path, SPX_N);
        }
        else
        {
            sphincsplus_thash_x2(buffer, buffer, pubseed, addr);
            qsc_memutils_copy(buffer + SPX_N, auth_path, SPX_N);
        }
    ...
    sphincsplus_set_tree_index(addr, leaf_idx + idx_offset);
    sphincsplus_thash_x2(root, buffer, pubseed, addr);
}

static void sphincsplus_treehash_root(uint8_t* root, uint8_t* auth_path, const uint8_t* sk_seed, const uint8_t* pubseed,
    ...
    void (*gen_leaf)(
        ...
        uint32_t,            /* addr_idx */
        const uint32_t[8U]),  /* tree_addr */
    uint32_t tree_addr[8U])
{
    ...
    QSC_SIMD_ALIGN uint32_t heights[SPX_TREE_HEIGHT + 1U] = { 0U };
    QSC_SIMD_ALIGN uint8_t stack[(SPX_TREE_HEIGHT + 1U) * SPX_N] = { 0U };
    uint32_t offset;
    uint32_t tree_idx;

    offset = 0U;

    for (uint32_t idx = 0U; idx < (uint32_t)(1U << SPX_TREE_HEIGHT); ++idx)
    {
        /* Add the next leaf node to the stack. */
        gen_leaf(stack + offset * SPX_N, sk_seed, pubseed, idx + idx_offset, tree_addr);
        ++offset;
        heights[offset - 1U] = 0U;

        /* If this is a node we need for the auth path.. */
        if ((leaf_idx ^ 0x01U) == idx)
        {
            qsc_memutils_copy(auth_path, stack + (offset - 1U) * SPX_N, SPX_N);
        }
        ...
        while (offset >= 2U && heights[offset - 1U] == heights[offset - 2U])
        {
            ...
            /* Set the address of the node we're creating. */
            sphincsplus_set_tree_height(tree_addr, heights[offset - 1U] + 1U);
            sphincsplus_set_tree_index(tree_addr, tree_idx + (idx_offset >> (heights[offset - 1U] + 1U)));
            /* Hash the top-most nodes from the stack together. */
            sphincsplus_thash_x2(stack + (offset - 2U) * SPX_N, stack + (offset - 2U) * SPX_N, pubseed, tree_addr);
            --offset;
            /* Note that the top-most node is now one layer higher. */
            ++heights[offset - 1U];

            /* If this is a node we need for the auth path.. */
            if (((leaf_idx >> heights[offset - 1U]) ^ 0x01U) == tree_idx)
            {
                qsc_memutils_copy(auth_path + heights[offset - 1U] * SPX_N, stack + (offset - 1U) * SPX_N, SPX_N);
            }
        }
    }
...
static void sphincsplus_treehash_fors(uint8_t* root, uint8_t* auth_path, const uint8_t* sk_seed, const uint8_t* pubseed,
    uint32_t leaf_idx, uint32_t idx_offset, 
    void (*gen_leaf)(
        uint8_t*,            /* leaf */
        const uint8_t*,      /* sk_seed */
    ...
    uint32_t tree_addr[8U])
{
    ...
    QSC_SIMD_ALIGN uint32_t heights[SPX_FORS_HEIGHT + 1U] = { 0U };
    QSC_SIMD_ALIGN uint8_t stack[(SPX_FORS_HEIGHT + 1U) * SPX_N] = { 0U };
    uint32_t offset;
    uint32_t tree_idx;

    offset = 0U;

    for (uint32_t idx = 0U; idx < (uint32_t)(1 << SPX_FORS_HEIGHT); ++idx)
    {
        /* Add the next leaf node to the stack. */
        gen_leaf(stack + offset * SPX_N, sk_seed, pubseed, idx + idx_offset, tree_addr);
        ++offset;
...
static void sphincsplus_fors_gen_sk(uint8_t* sk, const uint8_t* sk_seed, const uint32_t fors_leaf_addr[8U])
{
    sphincsplus_prf_addr(sk, sk_seed, fors_leaf_addr);
}

static void sphincsplus_fors_sk_to_leaf(uint8_t* leaf, const uint8_t* sk, const uint8_t* pubseed, uint32_t fors_leaf_addr[8U])
{
    sphincsplus_thash_x1(leaf, sk, pubseed, fors_leaf_addr);
}
...
static void sphincsplus_message_to_indices(uint32_t* indices, const uint8_t* m)
{
    ...
    for (size_t i = 0U; i < SPX_FORS_TREES; ++i)
    {
        ...
        for (size_t j = 0U; j < SPX_FORS_HEIGHT; ++j)
        {
            indices[i] ^= ((m[oft >> 3] >> (oft & 0x07U)) & 0x01U) << j;
            ++oft;
        }
    }
}

static void sphincsplus_fors_sign(uint8_t* sig, uint8_t* pk, const uint8_t* m, const uint8_t* sk_seed, const uint8_t* pubseed, const uint32_t fors_addr[8U])
{
    ...
    for (uint32_t i = 0U; i < SPX_FORS_TREES; ++i)
    {
        ...
        sphincsplus_treehash_fors(roots + i * SPX_N, sig, sk_seed, pubseed, indices[i], idx_offset, sphincsplus_fors_gen_leaf, fors_tree_addr);

        sig += SPX_N * SPX_FORS_HEIGHT;
    }

    /* Hash horizontally across all tree roots to derive the public key. */
    sphincsplus_thash_fors(pk, roots, pubseed, fors_pk_addr);
}

static void sphincsplus_fors_pk_from_sig(uint8_t* pk, const uint8_t* sig, const uint8_t* m, const uint8_t* pubseed, const uint32_t fors_addr[8U])
{
...
static void sphincsplus_wots_gen_sk(uint8_t* sk, const uint8_t* sk_seed, uint32_t wots_addr[8U])
{
    ...
    sphincsplus_set_hash_addr(wots_addr, 0U);

    /* Generate sk element. */
    sphincsplus_prf_addr(sk, sk_seed, wots_addr);
}

static void sphincsplus_gen_chain(uint8_t* out, const uint8_t* in, uint32_t start, uint32_t steps, const uint8_t* pubseed, uint32_t addr[8U])
{
    ...
    for (size_t i = start; i < (start + steps) && i < SPX_WOTS_W; ++i)
    {
        ...
        sphincsplus_thash_x1(out, out, pubseed, addr);
    }
}
...
static void sphincsplus_base_w(uint32_t* output, const int32_t out_len, const uint8_t* input)
{
    ...
    for (int32_t consumed = 0; consumed < out_len; ++consumed)
    {
        ...
        bits -= SPX_WOTS_LOGW;
        output[octr] = (total >> bits) & (SPX_WOTS_W - 1U);
        ++octr;
    }
}
...
static void sphincsplus_wots_gen_pk(uint8_t* pk, const uint8_t* sk_seed, const uint8_t* pubseed, uint32_t addr[8U])
{
    ...
    for (size_t i = 0U; i < SPX_WOTS_LEN; ++i)
    {
        ...
        sphincsplus_gen_chain(pk + i * SPX_N, pk + i * SPX_N, 0, SPX_WOTS_W - 1U, pubseed, addr);
    }
}

static void sphincsplus_wots_sign(uint8_t* sig, const uint8_t* msg, const uint8_t* sk_seed, const uint8_t* pubseed, uint32_t addr[8U])
{
    /* Takes a n-byte message and the 32-byte sk_see to compute a signature 'sig'. */

    QSC_SIMD_ALIGN uint32_t lengths[SPX_WOTS_LEN];
    ...
    for (size_t i = 0U; i < SPX_WOTS_LEN; ++i)
    {
        ...
        sphincsplus_gen_chain(sig + i * SPX_N, sig + i * SPX_N, 0U, lengths[i], pubseed, addr);
    }
}

static void sphincsplus_wots_pk_from_sig(uint8_t* pk, const uint8_t* sig, const uint8_t* msg, const uint8_t* pubseed, uint32_t addr[8U])
{
    ...
    {
        sphincsplus_set_chain_addr(addr, (uint32_t)i);
        sphincsplus_gen_chain(pk + i * SPX_N, sig + i * SPX_N, lengths[i], SPX_WOTS_W - 1U - lengths[i], pubseed, addr);
    }
}

static void sphincsplus_wots_gen_leaf(uint8_t* leaf, const uint8_t* sk_seed, const uint8_t* pubseed, uint32_t addr_idx, const uint32_t tree_addr[8U])
{
    ...
    sphincsplus_thash_wots(leaf, pk, pubseed, wots_pk_addr);
}
...
bool sphincsplus_ref_generate_seed_keypair(uint8_t* pk, uint8_t* sk, const uint8_t* seed)
{
    ...
    sphincsplus_treehash_root(sk + 3U * SPX_N, auth_path, sk, sk + 2U * SPX_N, 0U, 0U, sphincsplus_wots_gen_leaf, top_tree_addr);
    ...
    *siglen = 0U;
    ...
    if (rng_generate(optrand, SPX_N))
    {
        ...
        for (size_t i = 0U; i < SPX_D; ++i)
        {
            ...
            sphincsplus_treehash_root(root, sig, sk_seed, pubseed, idx_leaf, 0U, sphincsplus_wots_gen_leaf, tree_addr);
...
bool sphincsplus_ref_sign_verify(const uint8_t* sig, size_t siglen, const uint8_t* m, size_t mlen, const uint8_t* pk)
{
    ...
    const QSC_SIMD_ALIGN uint8_t *pubseed = pk;
    const QSC_SIMD_ALIGN uint8_t *pub_root = pk + SPX_N;
    QSC_SIMD_ALIGN uint8_t mhash[SPX_FORS_MSG_BYTES];
    ...
    if (siglen == SPX_BYTES)
    {
        ...
        for (uint32_t i = 0U; i < SPX_D; ++i)
        {
            ...
            sphincsplus_thash_wots(leaf, wots_pk, pubseed, wots_pk_addr);

            /* Compute the root node of this subtree. */
            sphincsplus_compute_root(root, leaf, idx_leaf, 0U, sig, SPX_TREE_HEIGHT, pubseed, tree_addr);
            sig += SPX_TREE_HEIGHT * SPX_N;
...
bool sphincsplus_ref_sign(uint8_t* sm, size_t* smlen, const uint8_t* m, size_t mlen, const uint8_t* sk, bool (*rng_generate)(uint8_t*, size_t))
{
    ...
    *smlen = 0;
    ...
    if (sphincsplus_ref_sign_signature(sm, &siglen, m, mlen, sk, rng_generate))
    {
