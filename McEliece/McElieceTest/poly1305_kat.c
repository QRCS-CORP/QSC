...
bool poly1305_kat_test()
{
	...
	clear8(out, 16);
	poly1305_compute(out, msg1, 34, key[0]);

	if (are_equal8(out, exp[0], 16) == false)
	{
		status = false;
	}

	clear8(out, 16);
	poly1305_compute(out, msg2, 12, key[1]);

	if (are_equal8(out, exp[1], 16) == false)
	{
		status = false;
	}

	clear8(out, 16);
	poly1305_compute(out, msg3, 34, key[2]);

	if (are_equal8(out, exp[2], 16) == false)
	{
		status = false;
	}

	clear8(out, 16);
	poly1305_compute(out, msg4, 34, key[3]);

	if (are_equal8(out, exp[3], 16) == false)
	{
		status = false;
	}
	...
	poly1305_initialize(&ctx, key[0]);

	for (i = 0; i < 32; i += POLY1305_BLOCK_SIZE)
	{
		poly1305_blockupdate(&ctx, msg1 + i);
	}

	poly1305_update(&ctx, msg1 + i, 2);
	poly1305_finalize(&ctx, out);
