 ...
 */
int seedexpander_init(AES_XOF_struct* ctx, uint8_t* seed, uint8_t* diversifier, uint32_t maxlen)
{
 ...
 */
int seedexpander(AES_XOF_struct* ctx, uint8_t* x, uint32_t xlen)
{
	uint32_t offset;
	int i;
...
void AES256_ECB(uint8_t* key, uint8_t* ctr, uint8_t* buffer)
{
	...
	aes_ecb_encrypt(buffer, ctr, roundkeys, AES256_ROUNDKEY_DIMENSION);
