...
mqc_status crypto_kem_enc(uint8_t* ct, uint8_t* ss, const uint8_t* pk)
{
	...
	rstat = sysrand_getbytes(ss, MCELIECE_KEYBYTES);
#else
	...
	chacha_transform(&ctx, ct + MCELIECE_SYNDBYTES, ss, MCELIECE_KEYBYTES);
	poly1305_compute(ct + (MCELIECE_SYNDBYTES + MCELIECE_KEYBYTES), ct + MCELIECE_SYNDBYTES, 32, key + 32);
...
mqc_status crypto_kem_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk)
{
	...
	sha3_compute512(key, e, sizeof(e));
	astat = poly1305_verify(ct + MCELIECE_SYNDBYTES + MCELIECE_KEYBYTES, ct + MCELIECE_SYNDBYTES, 32, key + 32);

	chacha_state ctx;
	chacha_initialize(&ctx, key, 32, nonce);
	chacha_transform(&ctx, ss, ct + MCELIECE_SYNDBYTES, MCELIECE_KEYBYTES);
