/* 
  This file is for functions required for generating the control bits of the Benes network w.r.t. a random permutation
  see the Lev-Pippenger-Valiant paper https://www.computer.org/csdl/trans/tc/1981/02/06312171.pdf
*/

#include "controlbits.h"
#include "params.h"
#include <stdlib.h>

typedef char bit;

static bit is_smaller(uint32_t a, uint32_t b)
{
	uint32_t ret;

	ret = a - b;
	ret >>= 31;

	return (bit)ret;
}

static bit is_smaller_63b(uint64_t a, uint64_t b)
{
	uint64_t ret;

	ret = a - b;
	ret >>= 63;

	return (bit)ret;
}

static void cswap(uint32_t* x, uint32_t* y, bit swap)
{
	uint32_t m;
	uint32_t d;

	m = (uint32_t)swap;
	m = 0 - m;
	d = (*x ^ *y);
	d &= m;
	*x ^= d;
	*y ^= d;
}

static void cswap_63b(uint64_t* x, uint64_t* y, bit swap)
{
	uint64_t m;
	uint64_t d;

	m = (uint32_t)swap;
	m = 0 - m;
	d = (*x ^ *y);
	d &= m;
	*x ^= d;
	*y ^= d;
}

static void minmax(uint32_t* x, uint32_t* y)
{
	/* output x = min(input x,input y) */
	/* output y = max(input x,input y) */

	bit m;

	m = is_smaller(*y, *x);
	cswap(x, y, m);

}

static void minmax_63b(uint64_t* x, uint64_t* y)
{
	bit m;

	m = is_smaller_63b(*y, *x);
	cswap_63b(x, y, m);
}

static void merge(uint32_t n, uint32_t* x, uint32_t step)
{
	/* merge first half of x[0],x[step],...,x[(2*n-1)*step] with second half */
	/* requires n to be a power of 2 */

	size_t i;

	if (n == 1)
	{
		minmax(&x[0], &x[step]);
	}
	else 
	{
		merge(n / 2, x, step * 2);
		merge(n / 2, x + step, step * 2);

		for (i = 1; i < (2 * n) - 1; i += 2)
		{
			minmax(&x[i * step], &x[(i + 1) * step]);
		}
	}
}

static void merge_63b(uint32_t n, uint64_t* x, uint32_t step)
{
	size_t i;

	if (n == 1)
	{
		minmax_63b(&x[0], &x[step]);
	}
	else 
	{
		merge_63b(n / 2, x, step * 2);
		merge_63b(n / 2, x + step, step * 2);

		for (i = 1; i < 2 * n - 1; i += 2)
		{
			minmax_63b(&x[i * step], &x[(i + 1) * step]);
		}
	}
}

static void sort(uint32_t n, uint32_t* x)
{
	/* sort x[0],x[1],...,x[n-1] in place */
	/* requires n to be a power of 2 */

	if (n > 1)
	{
		sort(n / 2, x);
		sort(n / 2, x + (n / 2));
		merge(n / 2, x, 1UL);
	}
}

void sort_63b(uint32_t n, uint64_t* x)
{
	if (n > 1)
	{
		sort_63b(n / 2, x);
		sort_63b(n / 2, x + (n / 2));
		merge_63b(n / 2, x, 1UL);
	}
}

static void composeinv(uint32_t n, uint32_t* y, const uint32_t* x, const uint32_t* pi)
{
	/* y[pi[i]] = x[i] */
	/* requires n = 2^w */
	/* requires pi to be a permutation */

	size_t i;

#ifdef MQC_COMPILER_GCC
	uint8_t* t[n * sizeof(uint32_t)];
#else
	uint32_t* t = malloc(n * sizeof(uint32_t));
#endif

	if (t != NULL)
	{
		for (i = 0; i < n; ++i)
		{
			t[i] = x[i] | (pi[i] << 16);
		}

		sort(n, t);

		for (i = 0; i < n; ++i)
		{
			y[i] = t[i] & 0x0000FFFFUL;
		}

#ifndef MQC_COMPILER_GCC
		free(t);
#endif
	}
}

static void invert(uint32_t n, uint32_t* ip, const uint32_t* pi)
{
	/* ip[i] = j iff pi[i] = j */
	/* requires n = 2^w */
	/* requires pi to be a permutation */

	uint32_t i;

	for (i = 0; i < n; i++)
	{
		ip[i] = i;
	}

	composeinv(n, ip, ip, pi);
}

static void flow(uint32_t w, uint32_t* x, const uint32_t* y, const uint32_t t)
{
	uint32_t b;
	uint32_t ycopy;
	bit m0;
	bit m1;

	ycopy = *y;
	m0 = is_smaller(*y & ((1UL << w) - 1), *x & ((1UL << w) - 1));
	m1 = is_smaller(0UL, t);

	cswap(x, &ycopy, m0);
	b = m0 & m1;
	*x ^= (b << w);
}

static void compose(uint32_t w, uint32_t n, const uint32_t* pi, uint32_t* P)
{
	size_t i;
	uint32_t t;

#ifdef MQC_COMPILER_GCC
	uint32_t* I[2 * n * sizeof(uint32_t)];
	uint32_t* ip[n * sizeof(uint32_t)];
#else
	uint32_t* I = malloc(2 * n * sizeof(uint32_t));
	uint32_t* ip = malloc(n * sizeof(uint32_t));
#endif

	if (I != NULL && ip != NULL)
	{
		invert(n, ip, pi);

		for (i = 0; i < n; ++i)
		{
			I[i] = ip[i] | (1UL << w);
			I[n + i] = pi[i];
		}

		// end ip
		for (i = 0; i < 2 * n; ++i)
		{
			P[i] = (i >> w) + (i & ((1UL << w) - 2)) + ((i & 1UL) << w);
		}

#ifdef MQC_COMPILER_GCC
		uint32_t* PI[2 * n * sizeof(uint32_t)];
		uint32_t* T[2 * n * sizeof(uint32_t)];
#else
		uint32_t* PI = malloc(2 * n * sizeof(uint32_t));
		uint32_t* T = malloc(2 * n * sizeof(uint32_t));
#endif

		if (PI != NULL && T != NULL)
		{
			for (t = 0; t < w; ++t)
			{
				composeinv(2 * n, PI, P, I);

				for (i = 0; i < 2 * n; ++i)
				{
					flow(w, &P[i], &PI[i], t);
				}

				for (i = 0; i < 2 * n; ++i)
				{
					T[i] = I[i ^ 1];
				}

				composeinv(2 * n, I, I, T);

				for (i = 0; i < 2 * n; ++i)
				{
					T[i] = P[i ^ 1];
				}

				for (i = 0; i < 2 * n; ++i)
				{
					flow(w, &P[i], &T[i], 1);
				}
			}
		}

#ifndef MQC_COMPILER_GCC
		if (PI != NULL)
		{
			free(PI);
		}
		if (T != NULL)
		{
			free(T);
		}
#endif
	}

#ifndef MQC_COMPILER_GCC
	if (I != NULL)
	{
		free(I);
	}
	if (ip != NULL)
	{
		free(ip);
	}
#endif
}

static void permute(uint32_t w, uint32_t n, uint32_t off, uint32_t step, const uint32_t* P, const uint32_t* pi, uint8_t* c, uint32_t* piflip)
{
	size_t i;
	size_t j;

	for (i = 0; i < n; ++i)
	{
		for (j = 0; j < w; ++j)
		{
			piflip[i] = pi[i];
		}
	}

	for (i = 0; i < n / 2; ++i)
	{
		c[(off + i * step) / 8] |= ((P[i * 2] >> w) & 1) << ((off + i * step) % 8);
	}

	for (i = 0; i < n / 2; ++i)
	{
		c[(off + ((w - 1)*n + i) * step) / 8] |= ((P[n + i * 2] >> w) & 1) << ((off + ((w - 1) * n + i) * step) % 8);
	}

	for (i = 0; i < n / 2; ++i)
	{
		cswap(&piflip[i * 2], &piflip[i * 2 + 1], (P[n + i * 2] >> w) & 1);
	}
}

static void permutecontrolbits(uint32_t w, uint32_t n, uint32_t step, uint32_t off, uint8_t* c, const uint32_t* pi)
{
	/* input: permutation pi */
	/* output: (2w-1)n/2 (or 0 if n==1) control bits c[0],c[step],c[2*step],... */
	/* requires n = 2^w */

	size_t i;

	if (w == 1)
	{
		c[off / 8] |= (pi[0] & 1) << (off % 8);
	}

	if (w > 1)
	{
#ifdef MQC_COMPILER_GCC
		uint32_t* piflip[n * sizeof(uint32_t)];
#else
		uint32_t* piflip = malloc(n * sizeof(uint32_t));
#endif

		if (piflip != NULL)
		{
#ifdef MQC_COMPILER_GCC
			uint32_t* P[2 * n * sizeof(uint32_t)];
#else
			uint32_t* P = malloc(2 * n * sizeof(uint32_t));
#endif

			if (P != NULL)
			{
				compose(w, n, pi, P);
				permute(w, n, off, step, P, pi, c, piflip);
#ifndef MQC_COMPILER_GCC
				free(P);
#endif
			}

#ifdef MQC_COMPILER_GCC
			uint32_t* subpi[n * sizeof(uint32_t)];
#else
			uint32_t* subpi = malloc(n * sizeof(uint32_t));
#endif

			if (subpi != NULL)
			{
				for (i = 0; i < n / 2; ++i)
				{
					subpi[i] = piflip[i * 2] >> 1;
				}

				for (i = 0; i < n / 2; ++i)
				{
					subpi[i + n / 2] = piflip[(i * 2) + 1] >> 1;
				}

#ifndef MQC_COMPILER_GCC
				free(piflip);
#endif
				permutecontrolbits(w - 1, n / 2, step * 2, off + step * (n / 2), c, subpi);
				permutecontrolbits(w - 1, n / 2, step * 2, off + step * ((n / 2) + 1), c, &subpi[n / 2]);

#ifndef MQC_COMPILER_GCC
				free(subpi);
#endif
			}
		}
	}
}

void controlbits(uint8_t* out, const uint32_t* pi)
{
	/* input: pi, a permutation*/
	/* output: out, control bits w.r.t. pi */

	uint8_t c[(((2 * GFBITS) - 1) * (1 << GFBITS)) / 16] = { 0 };
	size_t i;

	permutecontrolbits(GFBITS, (1UL << GFBITS), 1UL, 0UL, c, pi);

	for (i = 0; i < sizeof(c); i++)
	{
		out[i] = c[i];
	}
}

