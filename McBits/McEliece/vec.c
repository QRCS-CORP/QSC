...
{
	uint64_t result[2 * MCELIECE_GFBITS - 1];
	int32_t i;
	...
	for (i = 2 * MCELIECE_GFBITS - 2; i >= MCELIECE_GFBITS; i--) 
	{
		result[i - 9] ^= result[i];
		result[i - MCELIECE_GFBITS] ^= result[i];
	}
...
{
	uint64_t result[MCELIECE_GFBITS];
	size_t i;
...
{
	uint64_t tmp_11[MCELIECE_GFBITS];
	uint64_t tmp_1111[MCELIECE_GFBITS];

	vec_copy(out, in);
	vec_sq(out, out);
	vec_mul(tmp_11, out, in);
	vec_sq(out, tmp_11);
	vec_sq(out, out);
	vec_mul(tmp_1111, out, tmp_11);
	vec_sq(out, tmp_1111);
	vec_sq(out, out);
	vec_sq(out, out);
	vec_sq(out, out);
	vec_mul(out, out, tmp_1111);
	vec_sq(out, out);
	vec_sq(out, out);
	vec_mul(out, out, tmp_11);
	vec_sq(out, out);
	vec_mul(out, out, in);
	vec_sq(out, out);
}
