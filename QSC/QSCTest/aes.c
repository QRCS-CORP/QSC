...
static void aes_decrypt_block(qsc_aes_state* state, __m128i* output, const __m128i* input)
{
...
static void aes_encrypt_block(qsc_aes_state* state, __m128i* output, const __m128i* input)
{
	const size_t RNDCNT = state->roundkeylen - 2;
	size_t keyctr;
...
static void aes_le128to8(uint8_t* input, size_t inplen, __m128i* output, size_t outlen)
{
	size_t i;
	uint32_t tmpk;
...
void qsc_aes_cbc_decrypt(qsc_aes_state* state, uint8_t* output, size_t *outputlen, const uint8_t* input, size_t inputlen)
{
	...
	qsc_aes_cbc_decrypt_block(state, tmpb, (uint8_t*)(input + oft));
	len = qsc_pkcs7_padding_length(tmpb);
	qsc_memutils_copy((uint8_t*)(output + oft), tmpb, QSC_AES_BLOCK_SIZE - len);
...
void qsc_aes_cbc_encrypt(qsc_aes_state* state, uint8_t* output, const uint8_t* input, size_t inputlen)
{
	...
	if (inputlen != 0)
	{
		...
		if (inputlen < QSC_AES_BLOCK_SIZE)
		{
			qsc_pkcs7_add_padding(tmpb, QSC_AES_BLOCK_SIZE - inputlen);
		}

		qsc_aes_cbc_encrypt_block(state, (uint8_t*)(output + oft), tmpb);
	}
}
...
static void encryption_test()
{
	...
	uint8_t ncea[QSC_AES_BLOCK_SIZE] = { 0 };
	uint8_t nces[QSC_AES_BLOCK_SIZE] = { 0 };
	...
	qsc_memutils_copy(ncel, ncea, QSC_AES_BLOCK_SIZE);
	qsc_intutils_le8increment(ncea, QSC_AES_BLOCK_SIZE);
	qsc_memutils_copy((uint8_t*)(ncel + 16), ncea, QSC_AES_BLOCK_SIZE);
	qsc_intutils_le8increment(ncea, QSC_AES_BLOCK_SIZE);
	qsc_memutils_copy((uint8_t*)(ncel + 32), ncea, QSC_AES_BLOCK_SIZE);
	qsc_intutils_le8increment(ncea, QSC_AES_BLOCK_SIZE);
	qsc_memutils_copy((uint8_t*)(ncel + 48), ncea, QSC_AES_BLOCK_SIZE);
...
static void aes_add_roundkey(uint8_t* state, const uint32_t *skeys)
{
	...
	for (i = 0; i < QSC_AES_BLOCK_SIZE; i += sizeof(uint32_t))
	{
		k = *skeys;
...
static void aes_decrypt_block(qsc_aes_state* state, uint8_t* output, const uint8_t* input)
{
	const uint8_t* buf;
	uint8_t s[16];
	size_t i;
	...
	aes_add_roundkey(s, (uint32_t*)(state->roundkeys + (state->rounds << 2)));

	for (i = state->rounds - 1; i > 0; i--)
	{
		aes_invshift_rows(s);
		aes_invsub_bytes(s);
		aes_add_roundkey(s, (uint32_t*)(state->roundkeys + (i << 2)));
...
static void aes_encrypt_block(qsc_aes_state* state, uint8_t* output, const uint8_t* input)
{
	...
	{
		aes_sub_bytes(buf, s_box);
		aes_shift_rows(buf);
		aes_mix_columns(buf);
		aes_add_roundkey(buf, (uint32_t*)(state->roundkeys + (i << 2)));
	}

	aes_sub_bytes(buf, s_box);
	aes_shift_rows(buf);
	aes_add_roundkey(buf, (uint32_t*)(state->roundkeys + (state->rounds << 2)));
...
void qsc_aes_cbc_decrypt(qsc_aes_state* state, uint8_t* output, size_t *outputlen, const uint8_t* input, size_t inputlen)
{
	...
	while (inputlen > QSC_AES_BLOCK_SIZE)
	{
		qsc_aes_cbc_decrypt_block(state, (uint8_t*)(output + oft), (uint8_t*)(input + oft));
		inputlen -= QSC_AES_BLOCK_SIZE;
		oft += QSC_AES_BLOCK_SIZE;
	}
	...
	qsc_memutils_copy((uint8_t*)(output + oft), tmpb, QSC_AES_BLOCK_SIZE - nlen);
...
void qsc_aes_cbc_encrypt(qsc_aes_state* state, uint8_t* output, const uint8_t* input, size_t inputlen)
{
	...
	while (inputlen > QSC_AES_BLOCK_SIZE)
	{
		qsc_aes_cbc_encrypt_block(state, (uint8_t*)(output + oft), (uint8_t*)(input + oft));
		inputlen -= QSC_AES_BLOCK_SIZE;
		oft += QSC_AES_BLOCK_SIZE;
	}
...
void qsc_aes_ctrbe_transform(qsc_aes_state* state, uint8_t* output, const uint8_t* input, size_t inputlen)
{
	...
	while (inputlen >= QSC_AES_BLOCK_SIZE)
	{
		aes_encrypt_block(state, (uint8_t*)(output + oft), state->nonce);
