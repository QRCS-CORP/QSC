...
static falcon_prng_init(falcon_prng_state* pctx, qsc_keccak_state* kctx)
{
	/*
...
static const uint8_t falcon_max_fg_bits[FALCON_MAXBITS_SIZE] =
{
...
static const uint32_t falcon_l2bound[FALCON_L2BOUND_SIZE] =
{
...
static const falcon_fpr falcon_fpr_inv_sigma[FALCON_FPR_INV_SIGMA_SIZE] =
{
...
static const falcon_fpr falcon_fpr_sigma_min[FALCON_FPR_INV_SIGMA_SIZE] =
{
...
static const falcon_fpr falcon_fpr_gm_tab[FALCON_FPR_GM_TAB_SIZE] =
{
...
static const falcon_fpr falcon_fpr_p2_tab[FALCON_FPR_GM_P2_SIZE] =
{
...
static void falcon_iFFT(falcon_fpr* f, uint32_t logn)
{
	...
	for (u = logn; u > 1; u--)
	{
		...
		for (i1 = 0, j1 = 0; j1 < hn; i1++, j1 += dt)
		{
			...
			s_im = falcon_fpr_neg(falcon_fpr_gm_tab[((hm + i1) << 1) + 1]);
...
static void falcon_poly_split_fft(falcon_fpr* restrict f0, falcon_fpr* restrict f1, const falcon_fpr* restrict f, uint32_t logn)
{
	...
	*/
	...
	for (u = 0; u < qn; ++u)
	{
		...
		falcon_fpc_mul(&t_re, &t_im, t_re, t_im, falcon_fpr_gm_tab[((u + hn) << 1)], falcon_fpr_neg(falcon_fpr_gm_tab[((u + hn) << 1) + 1]));
...
static void falcon_poly_merge_fft(falcon_fpr* restrict f, const falcon_fpr* restrict f0, const falcon_fpr* restrict f1, uint32_t logn)
{
	...
	for (u = 0; u < qn; ++u)
	{
		...
		a_re = f0[u];
		a_im = f0[u + qn];
		falcon_fpc_mul(&b_re, &b_im, f1[u], f1[u + qn], falcon_fpr_gm_tab[((u + hn) << 1)], falcon_fpr_gm_tab[((u + hn) << 1) + 1]);
		falcon_fpc_add(&t_re, &t_im, a_re, a_im, b_re, b_im);
		f[(u << 1)] = t_re;
		f[(u << 1) + hn] = t_im;
		falcon_fpc_sub(&t_re, &t_im, a_re, a_im, b_re, b_im);
 ...
 *   falcon_iGMb[x] = R*((1/g)^rev(x)) mod q
 * Since g = 7, 1/g = 8778 mod 12289.
 */
static const uint16_t falcon_iGMb[FALCON_GMB_SIZE] =
{
...
static const falcon_small_prime falcon_small_primes[FALCON_SMALL_PRIME_SIZE] =
{
 ...
 */
static const uint16_t falcon_rev10[] =
{
 ...
 */
static const uint64_t falcon_gauss_1024_12289[FALCON_GAUS_1024_12289_SIZE] =
{
 ...
 * accordingly.
 */
static const size_t FALCON_MAX_BL_SMALL[] = { 1, 1, 2, 2, 4, 7, 14, 27, 53, 106, 209 };

static const size_t FALCON_MAX_BL_LARGE[] = { 2, 2, 5, 7, 12, 21, 40, 78, 157, 308 };
...
int32_t qsc_falcon_ref_generate_keypair(uint8_t* pk, uint8_t* sk, void (*rng_generate)(uint8_t*, size_t))
{
	...
	uint16_t h[512];
	uint8_t seed[48];
	qsc_keccak_state kctx;//QSC_KECCAK_256_RATE
	size_t u;
	size_t v;
...
int32_t qsc_falcon_ref_sign(uint8_t *sm, size_t *smlen, const uint8_t *m, size_t mlen, const uint8_t *sk, void (*rng_generate)(uint8_t*, size_t))
{
	...
	uint8_t esig[FALCON_CRYPTO_SIGNAURE_BYTES - 2 - sizeof(nonce)];
