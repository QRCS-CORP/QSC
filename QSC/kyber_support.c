...
void gen_matrix(qsc_kyber_polyvec* a, const uint8_t* seed, int32_t transposed)
{
	...
	for (i = 0; i < QSC_KYBER_K; ++i)
	{
		for (j = 0; j < QSC_KYBER_K; ++j)
		{
			...
			qsc_shake128_initialize(&kstate, extseed, QSC_KYBER_SYMBYTES + 2);
			qsc_shake128_squeezeblocks(&kstate, buf, maxnblocks);
			ctr = rej_uniform(a[i].vec[j].coeffs, QSC_KYBER_N, buf, maxnblocks * QSC_SHAKE_128_RATE);

			while (ctr < QSC_KYBER_N)
			{
				qsc_shake128_squeezeblocks(&kstate, buf, 1);
				ctr += rej_uniform(a[i].vec[j].coeffs + ctr, QSC_KYBER_N - ctr, buf, QSC_SHAKE_128_RATE);
			}
		}
	}
}
