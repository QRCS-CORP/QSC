...
* 
* Copyright (c) 2020 Digital Freedom Defence
...
* qsc_hba_rhx512_initialize(&state, &kp, true);
* // add the associated data
* qsc_hba_set_associated(&state, aad, sizeof(aad));
* // encrypt the message
* qsc_hba_rhx512_transform(&state, cpt, msg, MSG_LEN);
* \endcode
*
*
...
* qsc_hba_rhx512_initialize(&state, &kp, false);
* // add the associated data
* qsc_hba_set_associated(&state, aad, sizeof(aad));
* // authenticate and decrypt the cipher-text
* if (qsc_hba_rhx512_transform(&state, msg, cpt, CPT_LEN - QSC_HBA512_MAC_LENGTH) == false)
* {
...
*/
#ifndef QSC_SYSTEM_AESNI_ENABLED
...
*/
#ifndef QSC_RHX_SHAKE_EXTENSION
#	ifndef QSC_RHX_HKDF_EXTENSION
...
typedef struct
{
	...
	const uint8_t* cust;	/*!< the ciphers custom key */
