...
void challenge(poly* c, const uint8_t mu[DILITHIUM_CRH_SIZE], const polyveck *w1)
{
	...
	shake256_initialize(&kstate, inbuf, sizeof(inbuf));
	shake256_squeezeblocks(&kstate, outbuf, 1);
	signs = 0;
...
void dilithium_generate(uint8_t* publickey, uint8_t* secretkey)
{
	const uint8_t *key;
	const uint8_t *rho;
	...
	randombytes(seedbuf, 3 * DILITHIUM_SEED_SIZE);
	rho = seedbuf;
	...
	shake256(tr, DILITHIUM_CRH_SIZE, publickey, DILITHIUM_PUBLICKEY_SIZE);
	pack_sk(secretkey, rho, key, tr, &s1, &s2, &t0);
}

void dilithium_sign(uint8_t* signedmsg, size_t* smsglen, const uint8_t* message, size_t msglen, const uint8_t* secretkey)
{
	size_t i;
	uint32_t n;
	...
	/* Compute CRH(tr, msg) */
	shake256(mu, DILITHIUM_CRH_SIZE, signedmsg + DILITHIUM_SIGNATURE_SIZE - DILITHIUM_CRH_SIZE, DILITHIUM_CRH_SIZE + msglen);

#ifdef RANDOMIZED_SIGNING
	randombytes(rhoprime, DILITHIUM_CRH_SIZE);
#else
	shake256(rhoprime, DILITHIUM_CRH_SIZE, key, DILITHIUM_SEED_SIZE + DILITHIUM_CRH_SIZE);
#endif
		...
		*msglen = smsglen - DILITHIUM_SIGNATURE_SIZE;
		...
		if (bsig == 0)
		{
			...
			if (bsig == 0)
			{
				...
				shake256((uint8_t*)message + (DILITHIUM_SIGNATURE_SIZE - DILITHIUM_CRH_SIZE), DILITHIUM_CRH_SIZE, publickey, DILITHIUM_PUBLICKEY_SIZE);
				shake256(mu, DILITHIUM_CRH_SIZE, (uint8_t*)message + (DILITHIUM_SIGNATURE_SIZE - DILITHIUM_CRH_SIZE), DILITHIUM_CRH_SIZE + *msglen);
