...
void qsc_dilithium_poly_uniform(qsc_dilithium_poly* a, const uint8_t seed[QSC_DILITHIUM_SEED_SIZE], uint16_t nonce)
{
	...
	qsc_shake128_initialize(&kstate, tmps, QSC_DILITHIUM_SEED_SIZE + 2);
...
{
	const size_t NBLKS = ((QSC_DILITHIUM_N / 2 * (1U << QSC_DILITHIUM_SETABITS)) / (2 * QSC_DILITHIUM_ETA + 1) + QSC_SHAKE_128_RATE) / QSC_SHAKE_128_RATE;
	uint8_t buf[(((QSC_DILITHIUM_N / 2 * (1U << QSC_DILITHIUM_SETABITS)) / (2 * QSC_DILITHIUM_ETA + 1) + QSC_SHAKE_128_RATE) / QSC_SHAKE_128_RATE) * QSC_SHAKE_128_RATE];
	qsc_keccak_state kstate;
	uint8_t tmps[QSC_DILITHIUM_SEED_SIZE + 2];
	size_t buflen;
	size_t i;
	...
	while (ctr < QSC_DILITHIUM_N)
	{
		...
		ctr += rej_eta(a->coeffs + ctr, QSC_DILITHIUM_N - ctr, buf, QSC_SHAKE_128_RATE);
	}
}
...
void qsc_dilithium_poly_uniform_gamma1m1(qsc_dilithium_poly* a, const uint8_t seed[QSC_DILITHIUM_CRH_SIZE], uint16_t nonce)
{
	...
	qsc_shake256_initialize(&kstate, tmps, QSC_DILITHIUM_CRH_SIZE + 2);
...
void qsc_dilithium_ksm_sign(uint8_t* signedmsg, size_t* smsglen, const uint8_t* message, size_t msglen, const uint8_t* secretkey, void (*rng_generate)(uint8_t*, size_t))
{
	size_t i;
	uint32_t n;
	...
	qsc_dilithium_unpack_sk(rho, key, tr, &s1, &s2, &t0, secretkey);
