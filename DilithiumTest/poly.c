...
{
	const size_t NBLKS = (769 + SHAKE128_RATE) / SHAKE128_RATE;
	uint8_t buf[(((769 + SHAKE128_RATE) / SHAKE128_RATE) * SHAKE128_RATE) + 2];
	...
	shake128_initialize(state, tmps, DILITHIUM_SEED_SIZE + 2);
	shake128_squeezeblocks(state, buf, NBLKS);
	...
	while (ctr < DILITHIUM_N) 
	{
		...
		shake128_squeezeblocks(state, buf + off, 1);
...
{
	const size_t NBLKS = ((DILITHIUM_N / 2 * (1U << DILITHIUM_SETABITS)) / (2 * DILITHIUM_ETA + 1) + SHAKE128_RATE) / SHAKE128_RATE;
	uint8_t buf[(((DILITHIUM_N / 2 * (1U << DILITHIUM_SETABITS)) / (2 * DILITHIUM_ETA + 1) + SHAKE128_RATE) / SHAKE128_RATE) * SHAKE128_RATE];
	uint64_t state[SHA3_STATESIZE] = { 0 };
	uint8_t tmps[DILITHIUM_SEED_SIZE + 2];
	size_t buflen;
	size_t ctr;
	...
	while (ctr < DILITHIUM_N) 
	{
		...
		ctr += rej_eta(a->coeffs + ctr, DILITHIUM_N - ctr, buf, SHAKE128_RATE);
	}
}
...
{
	const size_t NBLKS = (641 + SHAKE256_RATE) / SHAKE256_RATE;
	uint8_t buf[(((641 + SHAKE256_RATE) / SHAKE256_RATE) * SHAKE256_RATE) + 4];
	uint64_t state[SHA3_STATESIZE] = { 0 };
	...
	shake256_initialize(state, tmps, DILITHIUM_CRH_SIZE + 2);
	shake256_squeezeblocks(state, buf, NBLKS);
	buflen = NBLKS * SHAKE256_RATE;
	...
	while (ctr < DILITHIUM_N) 
	{
		...
		shake256_squeezeblocks(state, buf + off, 1);
...
void polyt1_unpack(poly* r, const uint8_t* a)
{
	...
	for (i = 0; i < DILITHIUM_N / 8; ++i) 
	{
		r->coeffs[(8 * i)] = ((a[(9 * i)] >> 0) | ((uint32_t)a[(9 * i) + 1] << 8)) & 0x000001FFUL;
		r->coeffs[(8 * i) + 1] = ((a[(9 * i) + 1] >> 1) | ((uint32_t)a[(9 * i) + 2] << 7)) & 0x000001FFUL;
		r->coeffs[(8 * i) + 2] = ((a[(9 * i) + 2] >> 2) | ((uint32_t)a[(9 * i) + 3] << 6)) & 0x000001FFUL;
		r->coeffs[(8 * i) + 3] = ((a[(9 * i) + 3] >> 3) | ((uint32_t)a[(9 * i) + 4] << 5)) & 0x000001FFUL;
		r->coeffs[(8 * i) + 4] = ((a[(9 * i) + 4] >> 4) | ((uint32_t)a[(9 * i) + 5] << 4)) & 0x000001FFUL;
		r->coeffs[(8 * i) + 5] = ((a[(9 * i) + 5] >> 5) | ((uint32_t)a[(9 * i) + 6] << 3)) & 0x000001FFUL;
		r->coeffs[(8 * i) + 6] = ((a[(9 * i) + 6] >> 6) | ((uint32_t)a[(9 * i) + 7] << 2)) & 0x000001FFUL;
		r->coeffs[(8 * i) + 7] = ((a[(9 * i) + 7] >> 7) | ((uint32_t)a[(9 * i) + 8] << 1)) & 0x000001FFUL;
	}
}
...
void polyt0_unpack(poly* r, const uint8_t* a) 
{
	...
	{
		r->coeffs[(4 * i)] = a[(7 * i)];
		r->coeffs[(4 * i)] |= (uint32_t)(a[(7 * i) + 1] & 0x3F) << 8;
...
void polyz_unpack(poly* r, const uint8_t* a)
{
	...
	for (i = 0; i < DILITHIUM_N / 2; ++i)
	{
		...
		r->coeffs[(2 * i)] |= (uint32_t)(a[(5 * i) + 2] & 0x0F) << 16;
