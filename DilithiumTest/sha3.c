...
void cshake128_simple_initialize(uint64_t* state, uint16_t custom, const uint8_t* seed, size_t seedlen)
{
	...
	SHAKE output if S and DILITHIUM_N = zero (sort of a customized custom-SHAKE function).
	Padding is hard-coded as the first 32 bits, plus 16 bits of fixed DILITHIUM_N, and 16 bits of counter C.
