...
* Contains the NewHope implementation wellness and kat test functions.
*
...
#include "../NewHope/util.h"
#include <cstdbool>
#include <stdint.h>
/*lint -e829 */
#include <stdio.h>
/* bogus integral type warnings, and stdio header warn */
/*lint -e731 */
/*lint -e970 */
/*lint -e747 */
...
bool test_keys()
{
	uint8_t ct[NEWHOPE_CIPHERTEXT_SIZE];
	uint8_t keya[NEWHOPE_SHAREDSECRET_SIZE];
	uint8_t keyb[NEWHOPE_SHAREDSECRET_SIZE];
	...
	for (i = 0; i < NEWHOPE_NTESTS; i++)
	{
		...
		if (crypto_kem_enc(ct, keyb, pk) != true)
		{
			state = false;
			break;
		}
		...
		if (crypto_kem_dec(keya, ct, sk) != true)
		{
			state = false;
			break;
		}

		if (are_equal8(keya, keyb, NEWHOPE_SHAREDSECRET_SIZE) != true)
		{
			state = false;
			break;
		}
...
bool test_invalid_sk_a()
{
	...
	for (i = 0; i < NEWHOPE_NTESTS; i++)
	{
		...
		if (sysrand_getbytes(sk, NEWHOPE_SECRETKEY_SIZE) != true)
		{
			state = false;
			break;
		}
...
bool test_invalid_ciphertext()
{
	...
	{
		/* get a random offset */
		if (sysrand_getbytes(pbuf, 1) != true)
		{
			state = false;
			break;
		}
		...
		ct[pos % NEWHOPE_SECRETKEY_SIZE] ^= 23;
...
* \return Returns true for test success
*/
bool newhope_kat_test()
{
	uint8_t ct[NEWHOPE_CIPHERTEXT_SIZE];
	uint8_t ct2[NEWHOPE_CIPHERTEXT_SIZE];
	uint8_t pk[NEWHOPE_PUBLICKEY_SIZE];
	uint8_t pk2[NEWHOPE_PUBLICKEY_SIZE];
	uint8_t seed[48];
	uint8_t sk[NEWHOPE_SECRETKEY_SIZE];
	uint8_t sk2[NEWHOPE_SECRETKEY_SIZE];
	uint8_t ssk1[NEWHOPE_SHAREDSECRET_SIZE];
	uint8_t ssk2[NEWHOPE_SHAREDSECRET_SIZE];
	uint8_t ssk3[NEWHOPE_SHAREDSECRET_SIZE];
	bool ret;

	/* NIST PQ Round 2, first known answer test vectors */
	hex_to_bin("91C6F9E25D905134C8CD16A6D98F110C0A64D4BD57B4936A3005CC52C69C85B8CEE660765FC08725F8046369CC80966ABFEC2057983DD88E65A2B0790C5CA92F"
		...
		"0C43B8BB389F1BEB6B021728ACDB1BE54CF5B8F5BCF7A40655FA8E776495AC3F", ct2, NEWHOPE_CIPHERTEXT_SIZE);
	hex_to_bin("5D45FF82825E94F186859A06B450F72CF013E93E65C348163772099AB01FAB739DAE061A6CAA67C83D6BC3E2854AB7FA5200A67F57CB350CF88F5628CCA54ECB"
		...
		"1C0EE1111B08003F28E65E8B3BDEB037CF8F221DFCDAF5950EDB38D506D85BEF", pk2, NEWHOPE_PUBLICKEY_SIZE);
	hex_to_bin("061550234D158C5EC95595FE04EF7A25767F2E24CC2BC479D09D86DC9ABCFDE7056A8C266F9EF97ED08541DBD2E1FFA1", seed, 48);
	hex_to_bin("AE1320B8A539B9BE069E05D91922F157194A462EBA2A01597BAA5C46941C60386A9A3DA0EA9AA4919DAD050E385A7A588B098A7765886244DD42A8D68C465FEB"
		...
		"8626ED79D451140800E03B59B956F8210E556067407D13DC90FA9E8B872BFB8F", sk2, NEWHOPE_SECRETKEY_SIZE);
	hex_to_bin("B6327BC9F7C2556E6F06061F4A9C2CD2F7AE89EBE73C00D1CE8CB3F1D0128DCE", ssk3, NEWHOPE_SHAREDSECRET_SIZE);
	...
	if (are_equal8(pk, pk2, NEWHOPE_PUBLICKEY_SIZE) != true)
	{
		ret = false;
	}

	/* compare the secret key to the expected output */
	if (are_equal8(sk, sk2, NEWHOPE_SECRETKEY_SIZE) != true)
	{
		ret = false;
	}

	/* derive a shared-secret key and creates a response (in: pk | out: ct and ssk2) */
	if (crypto_kem_enc(ct, ssk2, pk) != true)
	{
		ret = false;
	}

	/* compare the cipher-text to the expected output */
	if (are_equal8(ct, ct2, NEWHOPE_CIPHERTEXT_SIZE) != true)
	{
		ret = false;
	}

	/* a uses b's response to get the shared-secret key (in: ct, sk | out: ssk1) */
	if (crypto_kem_dec(ssk1, ct, sk) != true)
	{
		ret = false;
	}

	/* compare the two keys for equality */
	if (are_equal8(ssk1, ssk2, NEWHOPE_SYMBYTES) != true)
	{
		ret = false;
	}

	/* compare the key to the expected output */
	if (are_equal8(ssk1, ssk3, NEWHOPE_SYMBYTES) != true)
	{
		ret = false;
	}
...
/**
* \brief Run the NewHope implementation kat, stress, and correctness tests tests
*/
void newhope_test_run()
{
	if (newhope_kat_test() == true)
	{
		printf_s("Success! Passed NewHope known answer test. \n");
	}
	else
	{
		printf_s("Failure! Failed NewHope known answer test. \n \n");
	}

	if (test_keys() == true)
	{
		printf_s("Success! Passed NewHope key generation, encryption, and decryption stress test. \n");
	}
	else
	{
		printf_s("Failure! Failed NewHope key generation, encryption, and decryption stress test. \n \n");
	}

	if (test_invalid_sk_a() == true)
	{
		printf_s("Success! Passed NewHope secret key tamper test. \n");
	}
	else
	{
		printf_s("Failure! Failed NewHope secret key tamper test. \n");
	}

	if (test_invalid_ciphertext() == true)
	{
		printf_s("Success! Passed NewHope cipher-text tamper test. \n");
	}
	else
	{
		printf_s("Failure! Failed NewHope cipher-text tamper test. \n");
	}
}
