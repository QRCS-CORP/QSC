...
************************************************************/
static uint16_t gammas_bitrev_montgomery[NEWHOPE_N] =
{
...
************************************************************/
static uint16_t gammas_inv_montgomery[NEWHOPE_N] =
{
...
static uint16_t coeff_freeze(uint16_t x)
{
	...
	m = r - NEWHOPE_Q;
	c = (int16_t)m;
	/* jgu checked irrelevant warning */
	/*lint -save -e702*/
	c >>= 15;
	/*lint -restore */
	r = m ^ ((r ^ m) & c);
...
void poly_frombytes(poly* r, const uint8_t* a)
{
	...
	{
		r->coeffs[4 * i] = a[7 * i] | (((uint16_t)a[7 * i + 1] & 0x3f) << 8);
...
void poly_tobytes(uint8_t* r, const poly* p)
{
	...
	for (i = 0; i < NEWHOPE_N / 4; ++i)
	{
		...
		r[7 * i] = (uint8_t)(t0 & 0xFF);
		r[7 * i + 1] = (uint8_t)((t0 >> 8) | (t1 << 6));
		r[7 * i + 2] = (uint8_t)((t1 >> 2));
		r[7 * i + 3] = (uint8_t)((t1 >> 10) | (t2 << 4));
		r[7 * i + 4] = (uint8_t)((t2 >> 4));
		r[7 * i + 5] = (uint8_t)((t2 >> 12) | (t3 << 2));
		r[7 * i + 6] = (uint8_t)((t3 >> 6));
	}
}
...
void poly_decompress(poly* r, const uint8_t* a)
{
	...
	for (i = 0; i < NEWHOPE_N; i += 8)
	{
		...
		for (j = 0; j < 8; ++j)
		{
			r->coeffs[i + j] = (uint16_t)(((uint32_t)r->coeffs[i + j] * NEWHOPE_Q + 4) >> 3);
		}
		/*lint -restore */
	}
}
...
void poly_ntt(poly* r)
{
	...
	ntt(r->coeffs, gammas_bitrev_montgomery);
}
